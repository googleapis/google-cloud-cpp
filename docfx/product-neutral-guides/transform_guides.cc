...
std::string ExtractSnippet(std::string const& full_path, std::string const& tag) {
    std::string content;
    try {
    ...
    while (std::getline(snippet_ss, current_line)) {
        ...
        if (indent != std::string::npos && (min_indent == std::string::npos || indent < min_indent)) {
            min_indent = indent;
        }
    }

    // Apply the minimum indentation removal to all lines
    for (const auto& l : lines_to_process) {
...
int main(int argc, char* argv[]) {
    ...
    std::string markdown_dir = markdown_file.substr(0, markdown_file.find_last_of("/\\"));
    ...
    for (; it != end; ++it) {
        ...
        std::string full_snippet_path = markdown_dir + "/" + path;
    ...
    // 2. Transform relative links to absolute URLs
