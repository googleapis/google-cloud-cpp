...
CreateDefaultRequestIdServiceStub(
  ...
  auto service_grpc_stub = google::test::requestid::v1::RequestIdService::NewStub(channel);
  std::shared_ptr<RequestIdServiceStub> stub =
    std::make_shared<DefaultRequestIdServiceStub>(
      std::move(service_grpc_stub),
      google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<RequestIdServiceAuth>(
        std::move(auth), std::move(stub));
  }
  stub = std::make_shared<RequestIdServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<RequestIdServiceLogging>(
        std::move(stub),
