...
std::shared_ptr<DeprecatedServiceRestStub>
CreateDefaultDeprecatedServiceRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<DeprecatedServiceRestStub> stub =
      std::make_shared<DefaultDeprecatedServiceRestStub>(std::move(opts));
  stub = std::make_shared<DeprecatedServiceRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<DeprecatedServiceRestLogging>(
        std::move(stub),
