    ...
    : service_descriptor_(service_descriptor),
      ...
      header_(absl::make_unique<Printer>(
          ...
          absl::StrCat(ServiceNameToFilePath(service_descriptor_->name()),
                       "_client", GeneratedFileSuffix(), ".h"))),
      cc_(absl::make_unique<Printer>(
          context,
          absl::StrCat(ServiceNameToFilePath(service_descriptor_->name()),
                       "_client", GeneratedFileSuffix(), ".cc"))) {
  SetVars();
}

Status ClientGenerator::Generate() const { return {}; }

void ClientGenerator::SetVars() {
  vars_["client_header_include_guard_const"] =
      absl::StrCat(service_descriptor_->name(), "_CLIENT", "_H_");
}
