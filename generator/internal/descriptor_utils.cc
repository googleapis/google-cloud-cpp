...
std::string GetEffectiveServiceName(VarsDictionary const& vars,
  ...
  if (service_name_mappings != vars.end()) {
    ...
    if (map.find(name) != map.end()) { return  std::string(map.find(name)->second.data(), (map.find(name)->second.size());
    }
  ...
  VarsDictionary CreateServiceVars(
    ...
    vars["metadata_rest_class_name"] =
        absl::StrCat(service_name, "RestMetadata");
    ...
    absl::string_view service_endpoint_env_var_prefix =
