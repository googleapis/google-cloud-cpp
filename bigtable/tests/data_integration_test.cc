...
class DataTestEnvironment : public ::testing::Environment {
 public:
  DataTestEnvironment(std::string project, std::string instance,
  ...
  static std::string const& table_name() { return table_name_; }
...
void DataIntegrationTest::SetUp() {
  ...
  table_admin_ = absl::make_unique<bigtable::TableAdmin>(
      admin_client_, DataTestEnvironment::instance_id());
...
std::unique_ptr<bigtable::Table> DataIntegrationTest::CreateTable() {
  table_admin_->CreateTable(
      DataTestEnvironment::table_name(),
      bigtable::TableConfig({{family, bigtable::GcRule::MaxNumVersions(10)}},
                            {}));
  return absl::make_unique<bigtable::Table>(data_client_,
                                            DataTestEnvironment::table_name());
}
