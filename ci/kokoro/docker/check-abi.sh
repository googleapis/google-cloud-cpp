...
check_library() {
  ...
  (
    ...
    # We ignore all symbols in an "::internal::" namespace, because these are
    # not part of our public API. We do this by specifying a regex that matches
    # against the mangled symbol names. In this case, 8 is the number of
    # characters in the string "internal", and it should again be followed by
    # some other number indicating the length of the symbol within the
    # "internal" namespace. See: https://en.wikipedia.org/wiki/Name_mangling
    abi-compliance-checker \
      -skip-internal-symbols "8internal\d" \
