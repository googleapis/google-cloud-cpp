...
RUN curl -fsSL https://github.com/protocolbuffers/protobuf/archive/v24.1.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake \
        -DCMAKE_CXX_STANDARD=20 \
        -DCMAKE_BUILD_TYPE=Release \
...
RUN curl -fsSL https://github.com/google/re2/archive/2023-08-01.tar.gz | \
    tar -xzf - --strip-components=1 && \
...
RUN curl -fsSL https://github.com/grpc/grpc/archive/v1.58.0-pre1.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake \
        -DCMAKE_CXX_STANDARD=20 \
        -DCMAKE_BUILD_TYPE=Release \
...
WORKDIR /var/tmp/build/
RUN curl -fsSL https://github.com/open-telemetry/opentelemetry-cpp/archive/v1.11.0.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake \
        -DCMAKE_CXX_STANDARD=20 \
        -DCMAKE_BUILD_TYPE=Release \
