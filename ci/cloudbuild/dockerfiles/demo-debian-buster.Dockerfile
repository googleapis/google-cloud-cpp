...
RUN curl -sSL https://github.com/nlohmann/json/archive/v3.10.2.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake \
      -DCMAKE_BUILD_TYPE=Release \
      -DBUILD_SHARED_LIBS=yes \
...
RUN curl -sSL https://github.com/protocolbuffers/protobuf/archive/v3.18.0.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake \
        -DCMAKE_BUILD_TYPE=Release \
        -DBUILD_SHARED_LIBS=yes \
...
RUN curl -sSL https://github.com/grpc/grpc/archive/v1.40.0.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake \
        -DCMAKE_BUILD_TYPE=Release \
