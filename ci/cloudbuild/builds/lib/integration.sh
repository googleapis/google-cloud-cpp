...
function integration::bazel_generator_with_emulators() {
  ...
  if echo "${args[@]}" | grep -w -q -- "--config=msan"; then
    ...
    read -r protobuf_proto_path < <(find -L "${bazel_output_base}/external" -name "empty.proto" | sed -nE 's/(.+\/src)\/google\/protobuf\/empty.proto/\1/p')
    read -r googleapis_proto_path < <(find -L "${bazel_output_base}/external" -name "generator-versions.json" | sed -nE 's/(.+)\/generator-versions.json/\1/p')
