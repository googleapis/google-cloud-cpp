 ...
 * `-DGOOGLE_CLOUD_CPP_CLANG_TIDY=yes`: configure the build to run `clang-tidy`
   ...
   we use.
 * `CREATE_GRAPHVIZ`: if set to `yes` use `CMake` to generate a dependency graph
   of each target. This is useful when troubleshooting dependencies, or simply
   when trying to document them.
