...
std::shared_ptr<DataTransferServiceStub>
CreateDefaultDataTransferServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::bigquery::datatransfer::v1::DataTransferService::NewStub(channel);
  auto service_locations_stub = google::cloud::location::Locations::NewStub(channel);
  std::shared_ptr<DataTransferServiceStub> stub =
    std::make_shared<DefaultDataTransferServiceStub>(std::move(service_grpc_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DataTransferServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeDataTransferServiceTracingStub(std::move(stub));
  }
  return stub;
}
