...
std::shared_ptr<MigrationServiceStub> CreateDefaultMigrationServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_MIGRATION_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "bigquerymigration.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::bigquery::migration::v2::MigrationService::NewStub(
            channel);
    stub = std::make_shared<DefaultMigrationServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeMigrationServiceTracingStub(std::move(stub));
  }
  return stub;
}
