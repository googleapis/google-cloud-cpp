...
#ifndef GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_BIGQUERY_JOB_V2_INTERNAL_JOB_REST_CONNECTION_IMPL_H
#define GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_BIGQUERY_JOB_V2_INTERNAL_JOB_REST_CONNECTION_IMPL_H
...
class JobServiceRestConnectionImpl
 ...
 private:
  static std::unique_ptr<bigquery_job_v2::JobServiceRetryPolicy> retry_policy(
      Options const& options) {
    return options.get<bigquery_job_v2::JobServiceRetryPolicyOption>()->clone();
  }

  static std::unique_ptr<BackoffPolicy> backoff_policy(Options const& options) {
    return options.get<bigquery_job_v2::JobServiceBackoffPolicyOption>()
        ->clone();
  }

  static std::unique_ptr<bigquery_job_v2::JobServiceConnectionIdempotencyPolicy>
  idempotency_policy(Options const& options) {
    return options
        .get<bigquery_job_v2::JobServiceConnectionIdempotencyPolicyOption>()
        ->clone();
  }

  std::unique_ptr<google::cloud::BackgroundThreads> background_;
  std::shared_ptr<bigquery_job_v2_internal::JobServiceRestStub> stub_;
  Options options_;
};
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_BIGQUERY_JOB_V2_INTERNAL_JOB_REST_CONNECTION_IMPL_H
