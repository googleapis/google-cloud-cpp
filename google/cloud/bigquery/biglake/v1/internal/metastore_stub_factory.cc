...
std::shared_ptr<MetastoreServiceStub> CreateDefaultMetastoreServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_METASTORE_SERVICE_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "biglake.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::bigquery::biglake::v1::MetastoreService::NewStub(
            channel);
    stub = std::make_shared<DefaultMetastoreServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeMetastoreServiceTracingStub(std::move(stub));
  }
  return stub;
}
