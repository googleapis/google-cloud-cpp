...
std::shared_ptr<AnalyticsHubServiceStub> CreateDefaultAnalyticsHubServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_ANALYTICS_HUB_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "analyticshub.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::bigquery::analyticshub::v1::AnalyticsHubService::NewStub(
            channel);
    stub = std::make_shared<DefaultAnalyticsHubServiceStub>(
