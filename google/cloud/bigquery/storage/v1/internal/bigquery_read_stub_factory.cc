...
std::shared_ptr<BigQueryReadStub>
CreateDefaultBigQueryReadStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::bigquery::storage::v1::BigQueryRead::NewStub(channel);
  std::shared_ptr<BigQueryReadStub> stub =
    std::make_shared<DefaultBigQueryReadStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<BigQueryReadAuth>(
