...
std::shared_ptr<ReservationServiceStub> CreateDefaultReservationServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_RESERVATION_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "bigqueryreservation.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::bigquery::reservation::v1::ReservationService::NewStub(
            channel);
    stub = std::make_shared<DefaultReservationServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeReservationServiceTracingStub(std::move(stub));
  }
  return stub;
}
