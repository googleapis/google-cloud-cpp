...
Options DataPolicyServiceDefaultOptions(Options options) {
  ...
  if (!options.has<
          bigquery_datapolicies_v2::DataPolicyServiceBackoffPolicyOption>()) {
    options.set<bigquery_datapolicies_v2::DataPolicyServiceBackoffPolicyOption>(
        ExponentialBackoffPolicy(
            std::chrono::seconds(0), std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling, kBackoffScaling)
            .clone());
  }
  if (!options.has<bigquery_datapolicies_v2::
                       DataPolicyServiceConnectionIdempotencyPolicyOption>()) {
    options.set<bigquery_datapolicies_v2::
                    DataPolicyServiceConnectionIdempotencyPolicyOption>(
