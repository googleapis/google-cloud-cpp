...
std::shared_ptr<DataPolicyServiceStub> CreateDefaultDataPolicyServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_DATA_POLICY_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "bigquerydatapolicy.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::bigquery::datapolicies::v1::DataPolicyService::NewStub(
            channel);
    stub = std::make_shared<DefaultDataPolicyServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeDataPolicyServiceTracingStub(std::move(stub));
  }
  return stub;
}
