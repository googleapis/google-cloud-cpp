...
std::shared_ptr<RecommenderStub>
CreateDefaultRecommenderStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::recommender::v1::Recommender::NewStub(channel);
  std::shared_ptr<RecommenderStub> stub =
    std::make_shared<DefaultRecommenderStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<RecommenderAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeRecommenderTracingStub(std::move(stub));
  }
  return stub;
}
