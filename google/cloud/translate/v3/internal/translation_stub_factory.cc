...
std::shared_ptr<TranslationServiceStub> CreateDefaultTranslationServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TRANSLATION_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "translate.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultTranslationServiceStub>(
