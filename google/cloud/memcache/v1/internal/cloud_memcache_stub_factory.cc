...
std::shared_ptr<CloudMemcacheStub> CreateDefaultCloudMemcacheStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CLOUD_MEMCACHE_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "memcache.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultCloudMemcacheStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeCloudMemcacheTracingStub(std::move(stub));
  }
  return stub;
}
