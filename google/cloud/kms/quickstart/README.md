# HOWTO: using the Cloud Key Management Service (KMS) API C++ client in your project

This directory contains small examples showing how to use the Cloud Key Management Service (KMS) API C++
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,kms]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/kms/quickstart
