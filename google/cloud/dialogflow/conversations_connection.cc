...
#include "google/cloud/dialogflow/conversations_options.h"
#include "google/cloud/dialogflow/internal/conversations_connection_impl.h"
#include "google/cloud/dialogflow/internal/conversations_option_defaults.h"
#include "google/cloud/dialogflow/internal/conversations_stub_factory.h"
...
std::shared_ptr<ConversationsConnection> MakeConversationsConnection(
  ...
  internal::CheckExpectedOptions<CommonOptionList, GrpcOptionList,
                                 ConversationsPolicyOptionList>(options,
                                                                __func__);
  options =
      dialogflow_internal::ConversationsDefaultOptions(std::move(options));
  auto background = internal::MakeBackgroundThreadsFactory(options)();
  auto stub = dialogflow_internal::CreateDefaultConversationsStub(
      background->cq(), options);
  return std::make_shared<dialogflow_internal::ConversationsConnectionImpl>(
...
MakeConversationsConnection(std::shared_ptr<ConversationsStub> stub,
                            Options options) {
  options = ConversationsDefaultOptions(std::move(options));
  auto background = internal::MakeBackgroundThreadsFactory(options)();
  return std::make_shared<dialogflow_internal::ConversationsConnectionImpl>(
      std::move(background), std::move(stub), std::move(options));
}
