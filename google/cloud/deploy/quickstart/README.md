# HOWTO: using the Google Cloud Deploy API C++ client in your project

This directory contains small examples showing how to use the Google Cloud Deploy API C++
...
Like most Google Cloud Platform (GCP) services, Google Cloud Deploy API requires that
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,deploy]
   ```
...
1. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/deploy/quickstart
...
[quickstart-link]: https://cloud.google.com/deploy/docs/deploy-app-gke
