...
std::shared_ptr<BudgetServiceStub>
CreateDefaultBudgetServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::billing::budgets::v1::BudgetService::NewStub(channel);
  std::shared_ptr<BudgetServiceStub> stub =
    std::make_shared<DefaultBudgetServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<BudgetServiceAuth>(
