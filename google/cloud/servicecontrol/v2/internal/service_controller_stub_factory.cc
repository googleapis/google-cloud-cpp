...
std::shared_ptr<ServiceControllerStub> CreateDefaultServiceControllerStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SERVICE_CONTROLLER_ENDPOINT"),
      std::move(endpoint_option), "servicecontrol.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::api::servicecontrol::v2::ServiceController::NewStub(channel);
    stub = std::make_shared<DefaultServiceControllerStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeServiceControllerTracingStub(std::move(stub));
  }
  return stub;
}
