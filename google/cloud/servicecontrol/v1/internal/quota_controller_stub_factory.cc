...
std::shared_ptr<QuotaControllerStub>
CreateDefaultQuotaControllerStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::api::servicecontrol::v1::QuotaController::NewStub(channel);
  std::shared_ptr<QuotaControllerStub> stub =
    std::make_shared<DefaultQuotaControllerStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<QuotaControllerAuth>(
