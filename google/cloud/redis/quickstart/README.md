# HOWTO: using the Google Cloud Memorystore for Redis API C++ client in your project

This directory contains small examples showing how to use the Google Cloud
Memorystore for Redis API C++ client library in your own project. These
...
Like most Google Cloud Platform (GCP) services, Google Cloud Memorystore for Redis API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,redis]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/redis/quickstart
...
[quickstart-link]: https://cloud.google.com/memorystore/docs/redis/quickstarts
