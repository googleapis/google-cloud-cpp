...
std::shared_ptr<ExportServiceStub>
CreateDefaultExportServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::devtools::cloudprofiler::v2::ExportService::NewStub(channel);
  std::shared_ptr<ExportServiceStub> stub =
    std::make_shared<DefaultExportServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ExportServiceAuth>(
