...
class WebRiskServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<WebRiskServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `WebRiskServiceConnection` that stops retrying after a
...
class WebRiskServiceLimitedErrorCountRetryPolicy
 ...
 public:
  WebRiskServiceLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<WebRiskServiceRetryPolicy> clone() const override {
    return std::make_unique<WebRiskServiceLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `WebRiskServiceConnection` that stops retrying after
/// some wall clock time has elapsed.
class WebRiskServiceLimitedTimeRetryPolicy : public WebRiskServiceRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<WebRiskServiceRetryPolicy> clone() const override {
    return std::make_unique<WebRiskServiceLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 *
 * For mocking, see `webrisk_v1_mocks::MockWebRiskServiceConnection`.
 */
 ...
 * - `google::cloud::webrisk_v1::WebRiskServicePolicyOptionList`
 *
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_WEBRISK_V1_WEB_RISK_CONNECTION_H
