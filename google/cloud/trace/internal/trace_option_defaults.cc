...
Options TraceServiceDefaultOptions(Options options) {
  if (!options.has<EndpointOption>()) {
    auto env = internal::GetEnv("GOOGLE_CLOUD_CPP_TRACE_SERVICE_ENDPOINT");
  ...
  if (!options.has<trace::TraceServiceRetryPolicyOption>()) {
    options.set<trace::TraceServiceRetryPolicyOption>(
        trace::TraceServiceLimitedTimeRetryPolicy(std::chrono::minutes(30))
            .clone());
  }
  if (!options.has<trace::TraceServiceBackoffPolicyOption>()) {
    options.set<trace::TraceServiceBackoffPolicyOption>(
        ExponentialBackoffPolicy(std::chrono::seconds(1),
                                 std::chrono::minutes(5), kBackoffScaling)
            .clone());
  }
  if (!options.has<trace::TraceServiceConnectionIdempotencyPolicyOption>()) {
    options.set<trace::TraceServiceConnectionIdempotencyPolicyOption>(
        trace::MakeDefaultTraceServiceConnectionIdempotencyPolicy());
  }
