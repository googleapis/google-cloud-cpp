# HOWTO: using the Discovery Engine API C++ client in your project

This directory contains small examples showing how to use the Discovery
Engine API C++ client library in your own project. These
...
Like most Google Cloud Platform (GCP) services, Discovery Engine API requires that
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/discoveryengine/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,discoveryengine]
   ```
...
[quickstart-link]: http://cloud/discovery-engine/docs/reference/libraries
