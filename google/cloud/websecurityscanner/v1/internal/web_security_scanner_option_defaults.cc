...
Options WebSecurityScannerDefaultOptions(Options options) {
  ...
  if (!options.has<websecurityscanner_v1::WebSecurityScannerRetryPolicyOption>()) {
  ...
  if (!options.has<websecurityscanner_v1::WebSecurityScannerBackoffPolicyOption>()) {
    options.set<websecurityscanner_v1::WebSecurityScannerBackoffPolicyOption>(
        ExponentialBackoffPolicy(std::chrono::seconds(0), std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling, kBackoffScaling).clone());
  }
  if (!options.has<websecurityscanner_v1::WebSecurityScannerConnectionIdempotencyPolicyOption>()) {
    options.set<websecurityscanner_v1::WebSecurityScannerConnectionIdempotencyPolicyOption>(
