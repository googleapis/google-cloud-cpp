...
#ifndef GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_INTERNAL_ASYNC_REST_RETRY_LOOP_H
...
/**
 * Implement an asynchronous retry loop for wrapped REST requests.
 *
 ...
 *
 * This class supports cancelling a retry loop. Without cancels this class would
 * require no synchronization, as each request, backoff timer, and their
 * callbacks can only occur in sequence. Cancel requests, however, may be
 * invoked by any thread and require some form of synchronization. The basic
 * idea is that the class maintains a future (called `pending_operation_`) that
 * represents the current pending operation, i.e., the current request or
 * backoff timer. Cancelling the loop may require cancelling the pending
 * operation.  Using such a `pending_operation_` future is prone to subtle race
 * conditions. This section explains how we ensure safety.
 *
 ...
 *
 * @code
 * 1: void AsyncRestRetryLoopImpl::F() {
 ...
 * 8: }
 * 9:
 * 10: void AsyncRestRetryLoopImpl::Cancel() {
 * ....
 * 11:   this->pending_operation_.cancel();
 * ....
 * }
 * 12:
 * 13: void AsyncRestRetryLoopImpl::Callback() {
 * ....
 * 14:   F();
 * ...
 * }
 ...
 *   - Each request or backoff timer is sequenced-after a call to
 *     `StartOperation()`, see `StartAttempt()` and `StartBackoff()`.
 *   - Each request or backoff timer must complete before the next one
...
class AsyncRestRetryLoopImpl
 ...
 public:
  ...
  AsyncRestRetryLoopImpl& operator=(AsyncRestRetryLoopImpl const&) = delete;
  AsyncRestRetryLoopImpl& operator=(AsyncRestRetryLoopImpl&&) = delete;
 ...
 private:
  ...
  void StartAttempt() {
    ...
    if (retry_policy_->IsExhausted()) {
      return SetDone(internal::RetryLoopError("Retry policy exhausted in",
  ...
  void OnAttempt(T result) {
    ...
    last_status_ = internal::GetResultStatus(std::move(result));
    if (idempotency_ == Idempotency::kNonIdempotent) {
      return SetDone(internal::RetryLoopError(
          "Error in non-idempotent operation", location_, last_status_));
    }
    if (!retry_policy_->OnFailure(last_status_)) {
      if (retry_policy_->IsPermanentFailure(last_status_)) {
        return SetDone(internal::RetryLoopError("Permanent error in", location_,
                                                last_status_));
      }
      return SetDone(internal::RetryLoopError("Retry policy exhausted in",
                                              location_, last_status_));
  ...
  void OnBackoff(TimerArgType tp) {
    ...
    if (!tp) {
      ...
      return SetDone(internal::RetryLoopError("Timer failure in", location_,
                                              std::move(tp).status()));
    }
    StartAttempt();
  }
  ...
  State SetDoneWithCancel(std::unique_lock<std::mutex> lk) {
    ...
    done_ = true;
    lk.unlock();
    result_.set_value(internal::RetryLoopError("Retry loop cancelled",
                                               location_, last_status_));
    return State{true, 0};
  }
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_INTERNAL_ASYNC_REST_RETRY_LOOP_H
