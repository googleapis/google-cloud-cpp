...
using ::google::cloud::testing_util::kP12KeyFileContents;
using ::google::cloud::testing_util::kP12ServiceAccountId;
...
void CheckInfoYieldsExpectedAssertion(ServiceAccountCredentialsInfo const& info,
  ...
  EXPECT_CALL(*mock_impl, Constructor(GoogleOAuthRefreshEndpoint(), _))
      .Times(1);
  EXPECT_CALL(
      *mock_impl,
  ...
  ServiceAccountCredentials<MockRestClient, FakeClock> credentials(info);
...
TEST_F(ServiceAccountCredentialsTest,
  ...
  auto info = ParseServiceAccountCredentials(kJsonKeyfileContents, "test");
  ASSERT_STATUS_OK(info);
  ServiceAccountCredentials<MockRestClient> credentials(*info);
  // Calls Refresh to obtain the access token for our authorization header.
  EXPECT_EQ(std::make_pair(std::string{"Authorization"},
...
TEST_F(ServiceAccountCredentialsTest, UsesCARootsInfo) {
  ...
  auto mock_impl = MockRestClient::SingleImpl();
  EXPECT_CALL(*mock_impl, Constructor)
      .WillOnce([&](std::string const& endpoint, Options const& options) {
        EXPECT_THAT(endpoint, Eq(GoogleOAuthRefreshEndpoint()));
        ASSERT_TRUE(options.has<CARootsFilePathOption>());
        EXPECT_THAT(options.get<CARootsFilePathOption>(), Eq(cainfo));
      });
  EXPECT_CALL(
      *mock_impl,
      Post(_, A<std::vector<std::pair<std::string, std::string>> const&>()))
  ...
  ServiceAccountCredentials<MockRestClient, FakeClock> credentials(
      *info, Options{}.set<CARootsFilePathOption>(cainfo));
  // Call Refresh to obtain the access token for our authorization header.
  auto authorization_header = credentials.AuthorizationHeader();
  ASSERT_STATUS_OK(authorization_header);
  EXPECT_EQ(std::make_pair(std::string{"Authorization"},
                           std::string{"Mock-Type fake-token"}),
...
TEST_F(ServiceAccountCredentialsTest, ParseSimpleP12) {
  ...
  internal::WriteBase64AsBinary(filename, kP12KeyFileContents);
  ...
  auto signed_blob = credentials.SignBlob(info->client_email, "test-blob");
...
TEST_F(ServiceAccountCredentialsTest, ParseP12MissingKey) {
  ...
  internal::WriteBase64AsBinary(filename, kP12KeyFileMissingKey);
  auto info = ParseServiceAccountP12File(filename);
...
TEST_F(ServiceAccountCredentialsTest, ParseP12MissingCerts) {
  ...
  internal::WriteBase64AsBinary(filename, kP12KeyFileMissingCerts);
  auto info = ParseServiceAccountP12File(filename);
  EXPECT_THAT(info, Not(IsOk()));
}
