...
struct AccumulateAllEventsType {
  using type = absl::variant<std::vector<typename Source::value_type>,
                             typename Source::error_type>;
};

template <typename Source>
using accumulate_all_events_t = typename AccumulateAllEventsType<Source>::type;
...
future<accumulate_all_events_t<absl::decay_t<Source>>> accumulate_all(
  ...
  struct Accumulator {
    ...
    void on_next(source_event_t v, promise<event_t> done) {
      struct Visitor {
        Accumulator* self;
        promise<event_t> done;
