...
StatusOr<ServiceAccountCredentialsInfo> ParseServiceAccountP12File(
  ...
  if (pkey_raw == nullptr) {
    return internal::InvalidArgumentError("No private key found in PKCS#12 file (" + source + ")", GCP_ERROR_INFO());
  }
  if (cert_raw == nullptr) {
    return internal::InvalidArgumentError("No certificate found in PKCS#12 file (" + source + ")", GCP_ERROR_INFO());
  }
