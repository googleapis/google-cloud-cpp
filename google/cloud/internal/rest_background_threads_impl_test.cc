...
TEST(AutomaticallyCreatedRestBackgroundThreads, IsActive) {
  AutomaticallyCreatedRestBackgroundThreads actual(1);
  EXPECT_EQ(2, actual.pool_size());

  std::cout << " main thread=" << std::this_thread::get_id() << std::endl;
  promise<std::thread::id> bg;
  actual.cq().RunAsync([&bg] {
    std::cout << " thread=" << std::this_thread::get_id()
              << " test async func set_value" << std::endl;
    bg.set_value(std::this_thread::get_id());
  });
  std::cout << " thread=" << std::this_thread::get_id() << " " << __func__
            << " get_future().get()" << std::endl;
