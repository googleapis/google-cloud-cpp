...
AsyncRestPollLongRunningOperation MakePoll(
  ...
  return [mock](CompletionQueue& cq, RestContext& context,
                google::longrunning::GetOperationRequest const& request) {
    return mock->AsyncGetOperation(cq, context, request);
  };
}
...
AsyncRestCancelLongRunningOperation MakeCancel(
  ...
  return [mock](CompletionQueue& cq, RestContext& context,
    ...
    return mock->AsyncCancelOperation(cq, context, request);
  };
}
...
TEST(AsyncRestPollingLoopTest, ImmediateCancel) {
  ...
  EXPECT_CALL(*mock, AsyncGetOperation)
      .WillOnce([](CompletionQueue&, RestContext&,
                   google::longrunning::GetOperationRequest const&) {
...
TEST(AsyncRestPollingLoopTest, PollThenExhaustedPollingPolicy) {
  ...
  EXPECT_CALL(*mock, AsyncGetOperation)
      .Times(AtLeast(2))
      .WillRepeatedly([&](CompletionQueue&, RestContext&,
                          google::longrunning::GetOperationRequest const&) {
        EXPECT_EQ(internal::CurrentOptions().get<StringOption>(),
...
TEST(AsyncRestPollingLoopTest, ConfigurePollContext) {
  google::longrunning::Operation starting_op;
  starting_op.set_name("test-op-name");
  ...
  EXPECT_CALL(*mock, AsyncGetOperation)
      .WillOnce([](CompletionQueue&,
                   RestContext& context,
                   google::longrunning::GetOperationRequest const&) {
        EXPECT_EQ(internal::CurrentOptions().get<StringOption>(), "ConfigurePollContext");
  ...
  promise<StatusOr<google::longrunning::Operation>> p;
  auto pending =
      AsyncRestPollingLoop(std::move(cq), p.get_future(), MakePoll(mock), MakeCancel(mock),
                       std::move(policy), "test-function");
  {
    internal::OptionsSpan overlay(Options{}.set<StringOption>("uh-oh"));
    pending.cancel();
