...
Status AsStatus(StatusCode status_code, std::string payload) {
...
Status AsStatus(HttpStatusCode http_status_code, std::string payload) {
  ...
  return AsStatus(status_code, std::move(payload));
}

Status AsStatus(RestResponse&& response) {
  auto const status_code = MapHttpCodeToStatus(response.StatusCode());
  if (status_code == StatusCode::kOk) return Status{};
  auto payload = rest_internal::ReadAll(std::move(response).ExtractPayload());
  if (!payload.ok()) return Status(status_code, {}, {});
  return AsStatus(status_code, std::move(*payload));
}
