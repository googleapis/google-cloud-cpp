...
class AsyncRetryLoopCancelTest : public ::testing::Test {
 protected:
  std::size_t CancelCount() { return sequencer_.CancelCount(); }
  ...
  future<TimerResult> SimulateDeadlineTimer(
      std::chrono::system_clock::time_point tp) {
    return sequencer_.PushBack("DeadlineTimer")
  ...
  promise<Status> WaitForTimer() {
    auto p = sequencer_.PopFrontWithName();
    EXPECT_THAT(p.second, AnyOf("DeadlineTimer", "RelativeTimer"));
    return std::move(p.first);
  }
