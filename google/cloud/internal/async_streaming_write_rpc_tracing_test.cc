...
TEST(AsyncStreamingWriteRpcTracing, FinishWithoutStart) {
  auto span_catcher = testing_util::InstallSpanCatcher();
...
TEST(AsyncStreamingWriteRpcTracing, FinishWithStart) {
  auto span_catcher = testing_util::InstallSpanCatcher();
...
TEST(AsyncStreamingWriteRpcTracing, UnstartedStreamShouldNotExtractMetadata) {
  auto span_catcher = testing_util::InstallSpanCatcher();
...
TEST(AsyncStreamingWriteRpcTracing, StartedStreamShouldExtractMetadata) {
  auto span_catcher = testing_util::InstallSpanCatcher();
  {
    auto span = MakeSpan("span");
    auto mock = std::make_unique<MockStream>();
