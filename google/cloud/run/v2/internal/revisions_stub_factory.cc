...
std::shared_ptr<RevisionsStub>
CreateDefaultRevisionsStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::run::v2::Revisions::NewStub(channel);
  std::shared_ptr<RevisionsStub> stub =
    std::make_shared<DefaultRevisionsStub>(
      std::move(service_grpc_stub),
      google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<RevisionsAuth>(
