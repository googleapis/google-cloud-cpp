...
#include "google/cloud/run/v2/internal/tasks_tracing_stub.h"
...
std::shared_ptr<TasksStub> CreateDefaultTasksStub(
    google::cloud::CompletionQueue cq, Options& options) {
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TASKS_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultTasksStub>(std::move(service_grpc_stub));
  }
