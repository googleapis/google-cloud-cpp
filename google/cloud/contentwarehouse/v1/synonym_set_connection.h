...
class SynonymSetServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  ...
  /// handled.
  virtual std::unique_ptr<SynonymSetServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `SynonymSetServiceConnection` that stops retrying after a
...
class SynonymSetServiceLimitedErrorCountRetryPolicy
 ...
 public:
  SynonymSetServiceLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<SynonymSetServiceRetryPolicy> clone() const override {
    return std::make_unique<SynonymSetServiceLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `SynonymSetServiceConnection` that stops retrying after
...
class SynonymSetServiceLimitedTimeRetryPolicy
 ...
 public:
  ...
  std::unique_ptr<SynonymSetServiceRetryPolicy> clone() const override {
    return std::make_unique<SynonymSetServiceLimitedTimeRetryPolicy>(
        maximum_duration());
  }
...
class SynonymSetServiceConnection {
 public:
  virtual ~SynonymSetServiceConnection() = 0;
 ...
 * - `google::cloud::contentwarehouse_v1::SynonymSetServicePolicyOptionList`
 *
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_CONTENTWAREHOUSE_V1_SYNONYM_SET_CONNECTION_H
