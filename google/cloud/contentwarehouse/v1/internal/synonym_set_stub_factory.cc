...
std::shared_ptr<SynonymSetServiceStub> CreateDefaultSynonymSetServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SYNONYM_SET_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::contentwarehouse::v1::SynonymSetService::NewStub(
            channel);
    stub = std::make_shared<DefaultSynonymSetServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeSynonymSetServiceTracingStub(std::move(stub));
  }
  return stub;
}
