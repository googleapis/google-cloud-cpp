...
std::shared_ptr<DocumentSchemaServiceStub>
CreateDefaultDocumentSchemaServiceStub(google::cloud::CompletionQueue cq,
                                       Options& options) {
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_DOCUMENT_SCHEMA_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::contentwarehouse::v1::DocumentSchemaService::NewStub(
            channel);
    stub = std::make_shared<DefaultDocumentSchemaServiceStub>(
        std::move(service_grpc_stub));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DocumentSchemaServiceAuth>(std::move(auth),
                                                       std::move(stub));
  }
  stub = std::make_shared<DocumentSchemaServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<DocumentSchemaServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeDocumentSchemaServiceTracingStub(std::move(stub));
  }
  return stub;
}
