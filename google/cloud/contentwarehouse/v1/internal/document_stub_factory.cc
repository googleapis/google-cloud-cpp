...
CreateDefaultDocumentServiceStub(
  ...
  auto service_grpc_stub = google::cloud::contentwarehouse::v1::DocumentService::NewStub(channel);
  auto service_operations_stub = google::longrunning::Operations::NewStub(channel);
  std::shared_ptr<DocumentServiceStub> stub =
    std::make_shared<DefaultDocumentServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub));
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeDocumentServiceTracingStub(std::move(stub));
  }
  return stub;
}
