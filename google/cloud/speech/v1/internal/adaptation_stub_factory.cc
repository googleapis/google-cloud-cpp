...
std::shared_ptr<AdaptationStub> CreateDefaultAdaptationStub(
    google::cloud::CompletionQueue cq, Options const& options) {
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_ADAPTATION_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "speech.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::speech::v1::Adaptation::NewStub(channel);
    stub =
        std::make_shared<DefaultAdaptationStub>(std::move(service_grpc_stub));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AdaptationAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<AdaptationMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<AdaptationLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeAdaptationTracingStub(std::move(stub));
  }
  return stub;
}
