...
std::shared_ptr<ReferenceListServiceStub> CreateDefaultReferenceListServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(options.get<EndpointOption>(),
                                     internal::MakeChannelArguments(options));
  auto service_grpc_stub =
      google::cloud::chronicle::v1::ReferenceListService::NewStub(channel);
  auto service_operations_stub =
      google::longrunning::Operations::NewStub(channel);
  std::shared_ptr<ReferenceListServiceStub> stub =
      std::make_shared<DefaultReferenceListServiceStub>(
          std::move(service_grpc_stub), std::move(service_operations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ReferenceListServiceAuth>(std::move(auth),
                                                      std::move(stub));
  }
