...
std::shared_ptr<VmMigrationStub> CreateDefaultVmMigrationStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_VM_MIGRATION_ENDPOINT"),
      std::move(endpoint_option), "vmmigration.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultVmMigrationStub>(
