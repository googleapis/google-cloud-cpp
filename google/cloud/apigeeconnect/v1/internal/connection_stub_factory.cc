...
std::shared_ptr<ConnectionServiceStub>
CreateDefaultConnectionServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::apigeeconnect::v1::ConnectionService::NewStub(channel);
  std::shared_ptr<ConnectionServiceStub> stub =
    std::make_shared<DefaultConnectionServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ConnectionServiceAuth>(
