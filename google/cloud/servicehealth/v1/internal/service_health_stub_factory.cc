...
std::shared_ptr<ServiceHealthStub> CreateDefaultServiceHealthStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(options.get<EndpointOption>(),
                                     internal::MakeChannelArguments(options));
  auto service_grpc_stub =
      google::cloud::servicehealth::v1::ServiceHealth::NewStub(channel);
  std::shared_ptr<ServiceHealthStub> stub =
      std::make_shared<DefaultServiceHealthStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub =
        std::make_shared<ServiceHealthAuth>(std::move(auth), std::move(stub));
  }
  std::multimap<std::string, std::string> fixed_metadata;
  stub = std::make_shared<ServiceHealthMetadata>(std::move(stub),
