...
std::shared_ptr<ServiceHealthStub>
CreateDefaultServiceHealthStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::servicehealth::v1::ServiceHealth::NewStub(channel);
  auto service_locations_stub = google::cloud::location::Locations::NewStub(channel);
  std::shared_ptr<ServiceHealthStub> stub =
    std::make_shared<DefaultServiceHealthStub>(std::move(service_grpc_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ServiceHealthAuth>(
        std::move(auth), std::move(stub));
  }
  stub = std::make_shared<ServiceHealthMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<ServiceHealthLogging>(
        std::move(stub),
        options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeServiceHealthTracingStub(std::move(stub));
  }
  return stub;
}
