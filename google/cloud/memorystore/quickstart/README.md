# HOWTO: using the Memorystore API C++ client in your project

This directory contains small examples showing how to use the Memorystore API
...
Like most Google Cloud Platform (GCP) services, Memorystore API requires that
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/memorystore/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,memorystore]
   ```
