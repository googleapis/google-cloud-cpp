...
std::shared_ptr<StorageInsightsStub> CreateDefaultStorageInsightsStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_STORAGE_INSIGHTS_ENDPOINT"),
      std::move(endpoint_option), "storageinsights.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultStorageInsightsStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeStorageInsightsTracingStub(std::move(stub));
  }
  return stub;
}
