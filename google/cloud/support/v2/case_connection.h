...
class CaseServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<CaseServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `CaseServiceConnection` that stops retrying after a
 ...
 */
class CaseServiceLimitedErrorCountRetryPolicy : public CaseServiceRetryPolicy {
 public:
  CaseServiceLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<CaseServiceRetryPolicy> clone() const override {
    return std::make_unique<CaseServiceLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `CaseServiceConnection` that stops retrying after
/// some wall clock time has elapsed.
class CaseServiceLimitedTimeRetryPolicy : public CaseServiceRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<CaseServiceRetryPolicy> clone() const override {
    return std::make_unique<CaseServiceLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 * sets in `CaseServiceClient`. This allows users to inject custom behavior
 ...
 */
class CaseServiceConnection {
 public:
  virtual ~CaseServiceConnection() = 0;
 ...
 * should be passed as an argument to the constructor of CaseServiceClient.
 *
 * The optional @p options argument may be used to configure aspects of the
 * returned `CaseServiceConnection`. Expected options are any of the types in
 * the following option lists:
 *
 * - `google::cloud::CommonOptionList`
 * - `google::cloud::GrpcOptionList`
 * - `google::cloud::UnifiedCredentialsOptionList`
 * - `google::cloud::support_v2::CaseServicePolicyOptionList`
 *
 * @note Unexpected options will be ignored. To log unexpected options instead,
 *     set `GOOGLE_CLOUD_CPP_ENABLE_CLOG=yes` in the environment.
 *
 * @param options (optional) Configure the `CaseServiceConnection` created by
 * this function.
 */
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_SUPPORT_V2_CASE_CONNECTION_H
