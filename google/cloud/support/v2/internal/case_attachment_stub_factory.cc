...
CreateDefaultCaseAttachmentServiceStub(google::cloud::CompletionQueue cq,
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CASE_ATTACHMENT_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "cloudsupport.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultCaseAttachmentServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeCaseAttachmentServiceTracingStub(std::move(stub));
  }
  return stub;
}
