...
std::shared_ptr<CaseServiceStub> CreateDefaultCaseServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CASE_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::support::v2::CaseService::NewStub(channel);
    stub =
        std::make_shared<DefaultCaseServiceStub>(std::move(service_grpc_stub));
  }
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeCaseServiceTracingStub(std::move(stub));
  }
  return stub;
}
