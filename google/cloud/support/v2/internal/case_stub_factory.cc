...
std::shared_ptr<CaseServiceStub>
CreateDefaultCaseServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::support::v2::CaseService::NewStub(channel);
  std::shared_ptr<CaseServiceStub> stub =
    std::make_shared<DefaultCaseServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<CaseServiceAuth>(
