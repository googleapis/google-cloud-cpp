...
std::shared_ptr<DirectAccessServiceStub> CreateDefaultDirectAccessServiceStub(
  ...
  auto service_grpc_stub =
      google::cloud::devicestreaming::v1::DirectAccessService::NewStub(channel);
  std::shared_ptr<DirectAccessServiceStub> stub =
      std::make_shared<DefaultDirectAccessServiceStub>(
          std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DirectAccessServiceAuth>(std::move(auth),
                                                     std::move(stub));
  }
