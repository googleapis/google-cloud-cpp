...
std::shared_ptr<DirectAccessServiceStub>
CreateDefaultDirectAccessServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::devicestreaming::v1::DirectAccessService::NewStub(channel);
  std::shared_ptr<DirectAccessServiceStub> stub =
    std::make_shared<DefaultDirectAccessServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DirectAccessServiceAuth>(
        std::move(auth), std::move(stub));
  }
  stub = std::make_shared<DirectAccessServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<DirectAccessServiceLogging>(
        std::move(stub),
        options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeDirectAccessServiceTracingStub(std::move(stub));
  }
  return stub;
}
