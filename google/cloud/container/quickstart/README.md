# HOWTO: using the Kubernetes Engine API C++ client in your project

This directory contains small examples showing how to use the Kubernetes Engine
...
Like most Google Cloud Platform (GCP) services, Kubernetes Engine API requires that
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,container]
   ```
...
1. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/container/quickstart
