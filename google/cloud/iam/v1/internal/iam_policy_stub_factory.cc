...
std::shared_ptr<IAMPolicyStub> CreateDefaultIAMPolicyStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_IAM_POLICY_ENDPOINT"),
      std::move(endpoint_option), "iam-meta-api.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub = google::iam::v1::IAMPolicy::NewStub(channel);
    stub = std::make_shared<DefaultIAMPolicyStub>(std::move(service_grpc_stub));
  }
