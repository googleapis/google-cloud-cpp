...
class IAMCredentialsRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<IAMCredentialsRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `IAMCredentialsConnection` that stops retrying after a
...
class IAMCredentialsLimitedErrorCountRetryPolicy
 ...
 public:
  IAMCredentialsLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<IAMCredentialsRetryPolicy> clone() const override {
    return std::make_unique<IAMCredentialsLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `IAMCredentialsConnection` that stops retrying after
/// some wall clock time has elapsed.
class IAMCredentialsLimitedTimeRetryPolicy : public IAMCredentialsRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<IAMCredentialsRetryPolicy> clone() const override {
    return std::make_unique<IAMCredentialsLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 *
 * For mocking, see `iam_credentials_v1_mocks::MockIAMCredentialsConnection`.
 */
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_IAM_CREDENTIALS_V1_IAM_CREDENTIALS_CONNECTION_H
