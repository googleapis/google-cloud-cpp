...
std::shared_ptr<IAMStub> CreateDefaultIAMStub(google::cloud::CompletionQueue cq,
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_IAM_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "iam.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultIAMStub>(std::move(service_grpc_stub));
  }
