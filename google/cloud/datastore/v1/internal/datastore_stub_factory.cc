...
std::shared_ptr<DatastoreStub>
CreateDefaultDatastoreStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::datastore::v1::Datastore::NewStub(channel);
  auto service_operations_stub = google::longrunning::Operations::NewStub(channel);
  std::shared_ptr<DatastoreStub> stub =
    std::make_shared<DefaultDatastoreStub>(std::move(service_grpc_stub), std::move(service_operations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DatastoreAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeDatastoreTracingStub(std::move(stub));
  }
  return stub;
}
