...
std::shared_ptr<ConfigStub> CreateDefaultConfigStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CONFIG_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "config.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::config::v1::Config::NewStub(channel);
    stub = std::make_shared<DefaultConfigStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeConfigTracingStub(std::move(stub));
  }
  return stub;
}
