# HOWTO: using the Cloud FTP API C++ client in your project

This directory contains small examples showing how to use the Cloud FTP API C++
...
Like most Google Cloud Platform (GCP) services, Cloud FTP API requires that your
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/ftp/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,ftp]
   ```
