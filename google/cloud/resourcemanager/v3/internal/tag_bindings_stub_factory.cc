...
std::shared_ptr<TagBindingsStub> CreateDefaultTagBindingsStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TAG_BINDINGS_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultTagBindingsStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<TagBindingsAuth>(std::move(auth), std::move(stub));
  }
