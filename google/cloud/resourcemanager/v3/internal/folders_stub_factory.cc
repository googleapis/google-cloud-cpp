...
std::shared_ptr<FoldersStub> CreateDefaultFoldersStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_FOLDERS_ENDPOINT"),
      std::move(endpoint_option), "cloudresourcemanager.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::resourcemanager::v3::Folders::NewStub(channel);
    stub = std::make_shared<DefaultFoldersStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeFoldersTracingStub(std::move(stub));
  }
  return stub;
}
