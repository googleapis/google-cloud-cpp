...
void CreateFolder(::google::cloud::storagecontrol_v2::StorageControlClient client,
  ...
  [](storagecontrol::StorageControlClient client,
      ...
      std::string const& folder_id) {
     auto const parent = std::string{"projects/_/buckets/"} + bucket_name;
     auto status = client.CreateFolder(parent, google::storage::control::v2::Folder{}, folder_id);
     if (!status) throw std::move(status).status();
     
     std::cout << "Created folder: " << status->name() << "\n";
  }
...
void DeleteFolder(::google::cloud::storagecontrol_v2::StorageControlClient client,
  ...
  [](storagecontrol::StorageControlClient client,
      std::string const& bucket_name,
      std::string const& folder_id) {
    auto const name = std::string{"projects/_/buckets/"} + bucket_name + std::string{"/folders/"} + folder_id;
...
void GetFolder(::google::cloud::storagecontrol_v2::StorageControlClient client,
  ...
  [](storagecontrol::StorageControlClient client,
      ...
      std::string const& folder_id) {
    auto const name = std::string{"projects/_/buckets/"} + bucket_name + std::string{"/folders/"} + folder_id;
    auto status = client.GetFolder(name);
    if (!status) throw std::move(status).status();

    std::cout << "Got folder: " << status->name() << "\n";
  }
...
void ListFolders(::google::cloud::storagecontrol_v2::StorageControlClient client,
  ...
  [](storagecontrol::StorageControlClient client,
      std::string const& bucket_name) {
    auto const parent = std::string{"projects/_/buckets/"} + bucket_name;
    auto stream_range = client.ListFolders(parent);
    for(auto status : stream_range) {
      if (!status) throw std::move(status).status();
...
void RenameFolder(::google::cloud::storagecontrol_v2::StorageControlClient client,
  ...
  [](storagecontrol::StorageControlClient client,
      std::string const& bucket_name,
      std::string const& source_folder_id,
      std::string const& dest_folder_id) {

    auto name = std::string{"projects/_/buckets/"} + bucket_name + std::string{"/folders/"} + source_folder_id;
    auto status_future = client.RenameFolder(name, dest_folder_id);
...
int main(int argc, char* argv[]) {
  ...
  namespace storagecontrol = ::google::cloud::storagecontrol_v2;
  using ClientCommand = std::function<void(::google::cloud::storagecontrol_v2::StorageControlClient,
