# HOWTO: execute Universe Domain demo examples

This directory contains small examples that leverage the experimental
universe_domain feature to query the supplied credentials and modify service
endpoints accordingly.

These instructions assume that you have some experience as a C++ developer and
that you have a working C++ toolchain (compiler, linker, etc.) installed on your
platform.
...
To run the examples you will need a working Google Cloud Platform (GCP) project.

## Configuring authentication for the C++ Client Library

These examples uses several different Google Cloud Platform (GCP) services. All
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/universe_domain/demo
   bazel build ...
   ```
...
1. Run the example, changing the placeholder(s) to appropriate values:
   ...
   ```bash
   bazel run :kms_demo -- [PROJECT_ID] [LOCATION_ID]
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,kms,universe_domain]
   ```
...
1. Run the example, changing the placeholder(s) to appropriate values:
   ...
   ```bash
   .build/kms_demo [PROJECT_ID] [LOCATION_ID]
   ```
