...
CreateDefaultPolicyTagManagerSerializationStub(
  ...
  auto service_grpc_stub = google::cloud::datacatalog::v1::PolicyTagManagerSerialization::NewStub(channel);
  ...
  std::shared_ptr<PolicyTagManagerSerializationStub> stub =
    std::make_shared<DefaultPolicyTagManagerSerializationStub>(std::move(service_grpc_stub), std::move(service_operations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<PolicyTagManagerSerializationAuth>(
