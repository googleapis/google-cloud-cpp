...
class DlpServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  ...
  /// handled.
  virtual std::unique_ptr<DlpServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `DlpServiceConnection` that stops retrying after a
...
class DlpServiceLimitedErrorCountRetryPolicy : public DlpServiceRetryPolicy {
 public:
  DlpServiceLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<DlpServiceRetryPolicy> clone() const override {
...
/// A retry policy for `DlpServiceConnection` that stops retrying after
