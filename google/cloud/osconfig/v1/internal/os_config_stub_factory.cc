...
std::shared_ptr<OsConfigServiceStub>
CreateDefaultOsConfigServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::osconfig::v1::OsConfigService::NewStub(channel);
  std::shared_ptr<OsConfigServiceStub> stub =
    std::make_shared<DefaultOsConfigServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<OsConfigServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeOsConfigServiceTracingStub(std::move(stub));
  }
  return stub;
}
