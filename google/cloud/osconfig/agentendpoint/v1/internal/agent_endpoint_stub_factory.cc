...
std::shared_ptr<AgentEndpointServiceStub>
CreateDefaultAgentEndpointServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::osconfig::agentendpoint::v1::AgentEndpointService::NewStub(channel);
  std::shared_ptr<AgentEndpointServiceStub> stub =
    std::make_shared<DefaultAgentEndpointServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AgentEndpointServiceAuth>(
