# HOWTO: using the OS Config API C++ client in your project

This directory contains small examples showing how to use the OS Config API C++
...
Like most Google Cloud Platform (GCP) services, OS Config API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,osconfig]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/osconfig/quickstart
...
[quickstart-link]: https://cloud.google.com/osconfig/docs/quickstart
