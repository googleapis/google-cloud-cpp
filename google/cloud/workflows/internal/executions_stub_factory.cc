...
#include "google/cloud/workflows/internal/executions_auth_decorator.h"
#include "google/cloud/workflows/internal/executions_logging_decorator.h"
...
std::shared_ptr<ExecutionsStub> CreateDefaultExecutionsStub(
  ...
  auto service_grpc_stub =
      google::cloud::workflows::executions::v1::Executions::NewStub(channel);
  std::shared_ptr<ExecutionsStub> stub =
      std::make_shared<DefaultExecutionsStub>(std::move(service_grpc_stub));
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeExecutionsTracingStub(std::move(stub));
  }
  return stub;
}
