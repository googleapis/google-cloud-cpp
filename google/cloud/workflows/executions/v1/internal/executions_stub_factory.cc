...
std::shared_ptr<ExecutionsStub>
CreateDefaultExecutionsStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::workflows::executions::v1::Executions::NewStub(channel);
  std::shared_ptr<ExecutionsStub> stub =
    std::make_shared<DefaultExecutionsStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ExecutionsAuth>(
