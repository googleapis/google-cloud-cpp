...
#include "google/cloud/pubsublite/internal/cursor_stub_factory.h"
...
std::shared_ptr<CursorServiceStub>
CreateDefaultCursorServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::pubsublite::v1::CursorService::NewStub(channel);
  auto service_operations_stub = google::longrunning::Operations::NewStub(channel);
  std::shared_ptr<CursorServiceStub> stub =
    std::make_shared<DefaultCursorServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<CursorServiceAuth>(
