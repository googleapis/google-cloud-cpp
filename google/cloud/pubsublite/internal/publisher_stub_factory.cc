...
std::shared_ptr<PublisherServiceStub>
CreateDefaultPublisherServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::pubsublite::v1::PublisherService::NewStub(channel);
  auto service_operations_stub = google::longrunning::Operations::NewStub(channel);
  std::shared_ptr<PublisherServiceStub> stub =
    std::make_shared<DefaultPublisherServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<PublisherServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakePublisherServiceTracingStub(std::move(stub));
  }
  return stub;
}
