...
std::shared_ptr<ContentServiceStub>
CreateDefaultContentServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dataplex::v1::ContentService::NewStub(channel);
  ...
  std::shared_ptr<ContentServiceStub> stub =
    std::make_shared<DefaultContentServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ContentServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeContentServiceTracingStub(std::move(stub));
  }
  return stub;
}
