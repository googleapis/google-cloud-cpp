...
std::shared_ptr<MessageCallback> MakeTestMessageCallback(
    std::shared_ptr<MessageCallback> mock, Options opts) {
  return MakeTracingMessageCallback(
      std::move(mock),
      opts.set<pubsub::SubscriptionOption>(TestSubscription()));
}
...
TEST(TracingMessageCallback, UserCallback) {
  ...
  auto message_callback = MakeTestMessageCallback(std::move(mock), Options{});
...
TEST(TracingMessageCallback, AddTracingAckHandlerWithOtelEnabled) {
  namespace sc = opentelemetry::trace::SemanticConventions;
  auto span_catcher = InstallSpanCatcher();
  auto mock_handler =
      std::make_unique<pubsub_testing::MockExactlyOnceAckHandlerImpl>();
  ...
  EXPECT_CALL(*mock, user_callback)
      .WillOnce([&](MessageCallback::MessageAndHandler m) {
        auto ack_id = m.ack_handler->ack_id();
        EXPECT_EQ(ack_id, "ack-id");
        auto f = m.ack_handler->ack();
        f.get();
      });
  auto message_callback = MakeTestMessageCallback(
      std::move(mock), Options{}.set<OpenTelemetryTracingOption>(true));
  ...
  EXPECT_THAT(spans, Contains(AllOf(SpanNamed("test-sub subscribe"))));
  EXPECT_THAT(
      spans, Contains(AllOf(SpanHasInstrumentationScope(), SpanKindIsInternal(),
                            SpanNamed("test-sub process"),
                            SpanHasAttributes(OTelAttribute<std::string>(
                                sc::kMessagingSystem, "gcp_pubsub")),
                            SpanWithParent(span))));
  EXPECT_THAT(spans, Contains(AllOf(SpanNamed("test-sub ack"))));
}

TEST(TracingMessageCallback, DoesNotAddTracingAckHandlerWithOtelDisabled) {
  namespace sc = opentelemetry::trace::SemanticConventions;
  auto span_catcher = InstallSpanCatcher();
  auto mock_handler =
      std::make_unique<pubsub_testing::MockExactlyOnceAckHandlerImpl>();
  ...
  auto message_callback = MakeTestMessageCallback(
      std::move(mock), Options{}.set<OpenTelemetryTracingOption>(false));

  message_callback->user_callback(std::move(m));
  span->End();

  auto spans = span_catcher->GetSpans();
  // Contains 2 spans: 1) created in the test, 2) generated by the callback
  EXPECT_THAT(spans, Contains(AllOf(SpanNamed("test-sub subscribe"))));
