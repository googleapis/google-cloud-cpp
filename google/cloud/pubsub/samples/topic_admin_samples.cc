...
void AutoRun(std::vector<std::string> const& argv) {
  ...
  auto const testdata_directory = auto const kinesis_topic_id =
  ...
  auto const kinesis_updated_gcp_service_account =
      google::cloud::internal::GetEnv(
          ...
          .value_or("");
  // Only run the samples if the environment variables are set.
  bool enable_kinesis =
      !(kinesis_stream_arn.empty() || kinesis_consumer_arn.empty() ||
        kinesis_aws_role_arm.empty() || kinesis_gcp_service_account.empty() ||
  ...
  if (enable_kinesis) {
    ...
    CreateTopicWithKinesisIngestion(
        ...
        {project_id, kinesis_topic_id, kinesis_stream_arn, kinesis_consumer_arn,
         kinesis_aws_role_arm, kinesis_gcp_service_account});
