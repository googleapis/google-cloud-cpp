...
CreateDefaultSubscriptionAdminStub(
  ...
  std::shared_ptr<SubscriptionAdminStub> stub =
    std::make_shared<DefaultSubscriptionAdminStub>(std::move(service_grpc_stub), std::move(service_iampolicy_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<SubscriptionAdminAuth>(
        std::move(auth), std::move(stub));
  }
  stub = std::make_shared<SubscriptionAdminMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<SubscriptionAdminLogging>(
        std::move(stub),
        options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeSubscriptionAdminTracingStub(std::move(stub));
  }
  return stub;
}
