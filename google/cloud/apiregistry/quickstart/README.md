# HOWTO: using the Cloud API Registry API C++ client in your project

This directory contains small examples showing how to use the Cloud API Registry
...
Like most Google Cloud Platform (GCP) services, Cloud API Registry API requires
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/apiregistry/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,apiregistry]
   ```
