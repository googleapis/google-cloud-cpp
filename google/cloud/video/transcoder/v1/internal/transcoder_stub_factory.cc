...
std::shared_ptr<TranscoderServiceStub>
CreateDefaultTranscoderServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::video::transcoder::v1::TranscoderService::NewStub(channel);
  std::shared_ptr<TranscoderServiceStub> stub =
    std::make_shared<DefaultTranscoderServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<TranscoderServiceAuth>(
