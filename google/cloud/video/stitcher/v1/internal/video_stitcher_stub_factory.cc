...
std::shared_ptr<VideoStitcherServiceStub> CreateDefaultVideoStitcherServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_VIDEO_STITCHER_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "videostitcher.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::video::stitcher::v1::VideoStitcherService::NewStub(
            channel);
    stub = std::make_shared<DefaultVideoStitcherServiceStub>(
