...
std::shared_ptr<LivestreamServiceStub>
CreateDefaultLivestreamServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::video::livestream::v1::LivestreamService::NewStub(channel);
  auto service_locations_stub = google::cloud::location::Locations::NewStub(channel);
  std::shared_ptr<LivestreamServiceStub> stub =
    std::make_shared<DefaultLivestreamServiceStub>(
      std::move(service_grpc_stub), std::move(service_locations_stub),
      google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<LivestreamServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeLivestreamServiceTracingStub(std::move(stub));
  }
  return stub;
}
