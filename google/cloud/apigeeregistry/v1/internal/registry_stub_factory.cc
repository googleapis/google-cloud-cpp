...
std::shared_ptr<RegistryStub> CreateDefaultRegistryStub(
  ...
  auto service_grpc_stub =
      google::cloud::apigeeregistry::v1::Registry::NewStub(channel);
  std::shared_ptr<RegistryStub> stub =
      std::make_shared<DefaultRegistryStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<RegistryAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<RegistryMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<RegistryLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeRegistryTracingStub(std::move(stub));
  }
  return stub;
}
