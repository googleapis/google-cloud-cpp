# HOWTO: using the Vector Search API C++ client in your project

This directory contains small examples showing how to use the Vector Search API
...
Like most Google Cloud Platform (GCP) services, Vector Search API requires that
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/vectorsearch/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,vectorsearch]
   ```
