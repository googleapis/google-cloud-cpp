...
std::shared_ptr<DatastreamStub> CreateDefaultDatastreamStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_DATASTREAM_ENDPOINT"),
      std::move(endpoint_option), "datastream.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultDatastreamStub>(
