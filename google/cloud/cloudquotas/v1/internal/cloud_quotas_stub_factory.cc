...
std::shared_ptr<CloudQuotasStub> CreateDefaultCloudQuotasStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(options.get<EndpointOption>(),
                                     internal::MakeChannelArguments(options));
  auto service_grpc_stub =
      google::api::cloudquotas::v1::CloudQuotas::NewStub(channel);
  std::shared_ptr<CloudQuotasStub> stub =
      std::make_shared<DefaultCloudQuotasStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<CloudQuotasAuth>(std::move(auth), std::move(stub));
  }
  std::multimap<std::string, std::string> fixed_metadata;
  stub = std::make_shared<CloudQuotasMetadata>(std::move(stub),
                                               std::move(fixed_metadata));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<CloudQuotasLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeCloudQuotasTracingStub(std::move(stub));
  }
  return stub;
}
