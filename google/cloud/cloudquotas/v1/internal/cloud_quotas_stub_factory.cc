...
std::shared_ptr<CloudQuotasStub>
CreateDefaultCloudQuotasStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::api::cloudquotas::v1::CloudQuotas::NewStub(channel);
  std::shared_ptr<CloudQuotasStub> stub =
    std::make_shared<DefaultCloudQuotasStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<CloudQuotasAuth>(
