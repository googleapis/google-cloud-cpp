# HOWTO: using the Cloud Quotas API C++ client in your project

This directory contains small examples showing how to use the Cloud Quotas API
...
(GCP) project with at least one additional service enabled. Additional
permissions required are detailed here: [Quota Permissions][quota-permissions].
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/cloudquotas/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,cloudquotas]
   ```
...
[quota-permissions]: https://cloud.google.com/docs/quota/permissions
