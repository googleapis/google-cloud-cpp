...
std::shared_ptr<IndexEndpointServiceStub> CreateDefaultIndexEndpointServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_INDEX_ENDPOINT_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::aiplatform::v1::IndexEndpointService::NewStub(channel);
    stub = std::make_shared<DefaultIndexEndpointServiceStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<IndexEndpointServiceAuth>(std::move(auth),
                                                      std::move(stub));
  }
  stub = std::make_shared<IndexEndpointServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<IndexEndpointServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeIndexEndpointServiceTracingStub(std::move(stub));
  }
  return stub;
}
