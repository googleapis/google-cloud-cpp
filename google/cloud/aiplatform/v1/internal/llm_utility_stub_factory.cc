...
std::shared_ptr<LlmUtilityServiceStub>
CreateDefaultLlmUtilityServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::aiplatform::v1::LlmUtilityService::NewStub(channel);
  ...
  std::shared_ptr<LlmUtilityServiceStub> stub =
    std::make_shared<DefaultLlmUtilityServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_iampolicy_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<LlmUtilityServiceAuth>(
