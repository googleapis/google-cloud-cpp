...
std::shared_ptr<TensorboardServiceStub> CreateDefaultTensorboardServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TENSORBOARD_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::aiplatform::v1::TensorboardService::NewStub(channel);
    stub = std::make_shared<DefaultTensorboardServiceStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<TensorboardServiceAuth>(std::move(auth),
                                                    std::move(stub));
  }
