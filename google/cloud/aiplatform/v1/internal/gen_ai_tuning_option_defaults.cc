...
Options GenAiTuningServiceDefaultOptions(std::string const& location, Options options) {
  ...
  if (!options.has<aiplatform_v1::GenAiTuningServicePollingPolicyOption>()) {
    options.set<aiplatform_v1::GenAiTuningServicePollingPolicyOption>(
        GenericPollingPolicy<
            ...
            options.get<aiplatform_v1::GenAiTuningServiceRetryPolicyOption>()->clone(),
            ExponentialBackoffPolicy(std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling).clone()).clone());
  }
  if (!options.has<aiplatform_v1::GenAiTuningServiceConnectionIdempotencyPolicyOption>()) {
    options.set<aiplatform_v1::GenAiTuningServiceConnectionIdempotencyPolicyOption>(
        aiplatform_v1::MakeDefaultGenAiTuningServiceConnectionIdempotencyPolicy());
  }
