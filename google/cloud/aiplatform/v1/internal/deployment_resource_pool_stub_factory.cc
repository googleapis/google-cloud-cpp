...
CreateDefaultDeploymentResourcePoolServiceStub(
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::aiplatform::v1::DeploymentResourcePoolService::NewStub(
            channel);
    stub = std::make_shared<DefaultDeploymentResourcePoolServiceStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DeploymentResourcePoolServiceAuth>(std::move(auth),
                                                               std::move(stub));
  }
  stub = std::make_shared<DeploymentResourcePoolServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<DeploymentResourcePoolServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeDeploymentResourcePoolServiceTracingStub(std::move(stub));
  }
  return stub;
}
