...
std::shared_ptr<FeaturestoreServiceStub> CreateDefaultFeaturestoreServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_FEATURESTORE_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::aiplatform::v1::FeaturestoreService::NewStub(channel);
    stub = std::make_shared<DefaultFeaturestoreServiceStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<FeaturestoreServiceAuth>(std::move(auth),
                                                     std::move(stub));
  }
  stub = std::make_shared<FeaturestoreServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<FeaturestoreServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeFeaturestoreServiceTracingStub(std::move(stub));
  }
  return stub;
}
