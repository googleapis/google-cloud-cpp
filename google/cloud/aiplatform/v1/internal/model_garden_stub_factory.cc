...
#include "google/cloud/aiplatform/v1/internal/model_garden_logging_decorator.h"
...
std::shared_ptr<ModelGardenServiceStub> CreateDefaultModelGardenServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_MODEL_GARDEN_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "aiplatform.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::aiplatform::v1::ModelGardenService::NewStub(channel);
    stub = std::make_shared<DefaultModelGardenServiceStub>(
        std::move(service_grpc_stub));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ModelGardenServiceAuth>(std::move(auth),
                                                    std::move(stub));
  }
  stub = std::make_shared<ModelGardenServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<ModelGardenServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeModelGardenServiceTracingStub(std::move(stub));
  }
  return stub;
}
