...
std::shared_ptr<ExperimentsStub>
CreateDefaultExperimentsStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dialogflow::cx::v3::Experiments::NewStub(channel);
  ...
  std::shared_ptr<ExperimentsStub> stub =
    std::make_shared<DefaultExperimentsStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ExperimentsAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeExperimentsTracingStub(std::move(stub));
  }
  return stub;
}
