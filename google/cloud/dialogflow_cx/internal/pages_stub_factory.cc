...
std::shared_ptr<PagesStub>
CreateDefaultPagesStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dialogflow::cx::v3::Pages::NewStub(channel);
  ...
  std::shared_ptr<PagesStub> stub =
    std::make_shared<DefaultPagesStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<PagesAuth>(
