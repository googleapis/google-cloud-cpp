...
std::shared_ptr<WebhooksStub>
CreateDefaultWebhooksStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dialogflow::cx::v3::Webhooks::NewStub(channel);
  ...
  std::shared_ptr<WebhooksStub> stub =
    std::make_shared<DefaultWebhooksStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<WebhooksAuth>(
