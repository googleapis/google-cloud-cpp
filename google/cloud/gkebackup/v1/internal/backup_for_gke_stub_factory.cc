...
std::shared_ptr<BackupForGKEStub> CreateDefaultBackupForGKEStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_BACKUP_FOR_GKE_ENDPOINT"),
      std::move(endpoint_option), "gkebackup.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultBackupForGKEStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeBackupForGKETracingStub(std::move(stub));
  }
  return stub;
}
