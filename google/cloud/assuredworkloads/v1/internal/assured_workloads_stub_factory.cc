...
CreateDefaultAssuredWorkloadsServiceStub(google::cloud::CompletionQueue cq,
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_ASSURED_WORKLOADS_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "assuredworkloads.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::assuredworkloads::v1::AssuredWorkloadsService::NewStub(
            channel);
    stub = std::make_shared<DefaultAssuredWorkloadsServiceStub>(
