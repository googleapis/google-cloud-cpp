...
CreateDefaultRapidMigrationAssessmentStub(google::cloud::CompletionQueue cq,
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_RAPID_MIGRATION_ASSESSMENT_ENDPOINT"),
      std::move(endpoint_option), "rapidmigrationassessment.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub = google::cloud::rapidmigrationassessment::v1::
        RapidMigrationAssessment::NewStub(channel);
    stub = std::make_shared<DefaultRapidMigrationAssessmentStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<RapidMigrationAssessmentAuth>(std::move(auth),
                                                          std::move(stub));
  }
  stub = std::make_shared<RapidMigrationAssessmentMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<RapidMigrationAssessmentLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeRapidMigrationAssessmentTracingStub(std::move(stub));
  }
  return stub;
}
