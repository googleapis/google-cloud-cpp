...
class IamCheckerRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<IamCheckerRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `IamCheckerConnection` that stops retrying after a
 ...
 */
class IamCheckerLimitedErrorCountRetryPolicy : public IamCheckerRetryPolicy {
 public:
  IamCheckerLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<IamCheckerRetryPolicy> clone() const override {
    return std::make_unique<IamCheckerLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `IamCheckerConnection` that stops retrying after
/// some wall clock time has elapsed.
class IamCheckerLimitedTimeRetryPolicy : public IamCheckerRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<IamCheckerRetryPolicy> clone() const override {
    return std::make_unique<IamCheckerLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 * - `google::cloud::policytroubleshooter_v1::IamCheckerPolicyOptionList`
 *
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_POLICYTROUBLESHOOTER_V1_IAM_CHECKER_CONNECTION_H
