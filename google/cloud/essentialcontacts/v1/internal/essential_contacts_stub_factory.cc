...
CreateDefaultEssentialContactsServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::essentialcontacts::v1::EssentialContactsService::NewStub(channel);
  std::shared_ptr<EssentialContactsServiceStub> stub =
    std::make_shared<DefaultEssentialContactsServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<EssentialContactsServiceAuth>(
