...
CreateDefaultEssentialContactsServiceStub(google::cloud::CompletionQueue cq,
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_ESSENTIAL_CONTACTS_SERVICE_ENDPOINT"),
      std::move(endpoint_option), "essentialcontacts.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::essentialcontacts::v1::EssentialContactsService::NewStub(
            channel);
    stub = std::make_shared<DefaultEssentialContactsServiceStub>(
        std::move(service_grpc_stub));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<EssentialContactsServiceAuth>(std::move(auth),
                                                          std::move(stub));
  }
  stub = std::make_shared<EssentialContactsServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<EssentialContactsServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeEssentialContactsServiceTracingStub(std::move(stub));
  }
  return stub;
}
