 ...
 * result of an operation to the application. For the public layer we use
 * `google::cloud::future<>` which offers a more powerful API, as the
...
class AsyncFutureFromCallback {
 public:
  explicit AsyncFutureFromCallback(promise<StatusOr<R>> p, char const* w)
      : promise_(std::move(p)), where_(w) {}

  void operator()(CompletionQueue& cq, R& result, grpc::Status& status) {
    if (!status.ok()) {
      promise_.set_value(MakeStatusFromRpcError(
          status.error_code(), where_ + (": " + status.error_message())));
      return;
    }
 ...
 * `google::protobuf::Empty`), we should use `future<Status>` to represent the
...
class AsyncFutureFromCallback<void> {
 public:
  explicit AsyncFutureFromCallback(promise<Status> p, char const* w)
      : promise_(std::move(p)), where_(w) {}
 ...
 *
 * Given a `promise<StatusOr<T>>` deduce the desired type of
 * `AsyncFutureFromCallback<T>` and returns a new instance.
 */
template <typename T>
AsyncFutureFromCallback<T> MakeAsyncFutureFromCallback(promise<StatusOr<T>> p,
                                                       const char* w) {
  return AsyncFutureFromCallback<T>(std::move(p), w);
}

/**
 * Creates a `AsyncFutureFromCallback<void>`.
 *
 * It's just an overload for handling `promise<Status>` case.
 */
