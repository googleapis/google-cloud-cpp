...
std::unique_ptr<TableSchemaMetric> OperationLatency::clone(
...
std::unique_ptr<TableSchemaMetric> AttemptLatency::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
...
void AttemptLatency2::PostCall(opentelemetry::context::Context const& context,
  ...
  auto attempt_elapsed = std::chrono::duration_cast<LatencyDuration>(
      p.attempt_end - attempt_start_);
  attempt_latencies2_->Record(
      attempt_elapsed.count(),
      IntoLabelMap(resource_labels_, data_labels_, {}, peer_info_labels_),
      context);
}

std::unique_ptr<TableSchemaMetric> AttemptLatency2::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
...
std::unique_ptr<TableSchemaMetric> RetryCount::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
...
std::unique_ptr<TableSchemaMetric> FirstResponseLatency::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
...
std::unique_ptr<TableSchemaMetric> ServerLatency::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
...
std::unique_ptr<TableSchemaMetric> ApplicationBlockingLatency::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
...
std::unique_ptr<TableSchemaMetric> ConnectivityErrorCount::clone(
    TableResourceLabels const& resource_labels,
    TableDataLabels const& data_labels) const {
