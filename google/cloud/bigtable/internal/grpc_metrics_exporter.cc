...
bool GrpcMetricsExporterRegistry::Register(std::string authority) {
  ...
  return known_authorities_.insert(std::move(authority)).second;
}

void GrpcMetricsExporterRegistry::Clear() {
  std::unique_lock<std::mutex> lk(mu_);
  known_authorities_.clear();
}
...
MonitoredResourceResult MakeMonitoredResource(
  ...
  if ((project_id.empty() || instance.empty()) &&
    ...
    if (!instances.empty()) {
      if (project_id.empty()) project_id = instances[0].project_id();
      if (instance.empty()) instance = instances[0].instance_id();
    }
  }
  if (project_id.empty()) {
