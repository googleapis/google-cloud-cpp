 ...
 * the `parsed` column family contains a `trade` column, the values of these
 * column are serialized `taq.Trade` protos.
 *
 ...
 * serialized `taq.TradeSequence` proto.
 */

/// Helper functions used in the shuffle_taq_trade example.
namespace {
...
int main(int argc, char* argv[]) try {
  ...
  std::string const input_table_id = "taq-trades";
...
void Shuffle(cbt::Table& input, cbt::Table& output, std::string begin,
  ...
  auto reader = input.ReadRows(
      ...
      F::ColumnRangeClosed("parsed", "trade", "trade"));
  ...
  for (auto const& row : reader) {
    ...
    if (key != current_key) {
      if (not current_key.empty()) {
        ...
        mutation.emplace_back(cbt::SetCell("taq", "trades",
    ...
    for (auto const& cell : row.cells()) {
      if (cell.family_name() != "parsed" or
          cell.column_qualifier() != "trade") {
        continue;
      }
      taq::Quote nbbo;
