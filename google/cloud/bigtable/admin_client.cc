 ...
 * connections need refreshing.
 */
class DefaultLoggingAdminClient : public google::cloud::bigtable::AdminClient {
 private:
  // Introduce an early `private:` section because this type is used to define
  // the public interface, it should not be part of the public interface.
  struct AdminTraits {
 ...
 public:
  ...
  DefaultLoggingAdminClient(std::string project,
                            google::cloud::bigtable::ClientOptions options)
      : project_(std::move(project)), impl_(std::move(options)) {}
  ...
  grpc::Status GenerateConsistencyToken(
    ...
    return LogWrapper(
        [this](grpc::ClientContext* context,
          ...
          return impl_.Stub()->GenerateConsistencyToken(context, request,
                                                        response);
        },
        context, request, response, __func__,
        impl_.Options().tracing_options());
...
std::shared_ptr<AdminClient> CreateDefaultAdminClient(std::string project,
  ...
  if (options.tracing_enabled("rpc")) {
    ...
    return std::make_shared<DefaultLoggingAdminClient>(std::move(project),
                                                       std::move(options));
  }
