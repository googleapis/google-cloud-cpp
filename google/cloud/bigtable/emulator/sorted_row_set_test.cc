...
TEST(SortedRowSet, SingleRange) {
  SortedRowSet srs;
  std::string expected_text = R"""(
start_key_closed: 'a'
end_key_closed: 'b'
)""";
  btproto::RowRange expected;
  ASSERT_TRUE(
      google::protobuf::TextFormat::ParseFromString(expected_text, &expected));

  srs.Insert(RowRange::Closed("a", "b").as_proto());
  ASSERT_EQ(1, srs.disjoint_ranges().size());
  ASSERT_THAT(expected, IsProtoEqual(*srs.disjoint_ranges().begin()));
}
