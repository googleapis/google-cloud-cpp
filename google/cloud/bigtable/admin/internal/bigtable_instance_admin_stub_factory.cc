...
CreateDefaultBigtableInstanceAdminStub(
  ...
  auto service_grpc_stub = google::bigtable::admin::v2::BigtableInstanceAdmin::NewStub(channel);
  std::shared_ptr<BigtableInstanceAdminStub> stub =
    std::make_shared<DefaultBigtableInstanceAdminStub>(
      std::move(service_grpc_stub),
      google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<BigtableInstanceAdminAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeBigtableInstanceAdminTracingStub(std::move(stub));
  }
  return stub;
}
