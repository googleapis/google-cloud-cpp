...
CreateDefaultAdvisoryNotificationsServiceStub(
  ...
  auto service_grpc_stub = google::cloud::advisorynotifications::v1::AdvisoryNotificationsService::NewStub(channel);
  std::shared_ptr<AdvisoryNotificationsServiceStub> stub =
    std::make_shared<DefaultAdvisoryNotificationsServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AdvisoryNotificationsServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeAdvisoryNotificationsServiceTracingStub(std::move(stub));
  }
  return stub;
}
