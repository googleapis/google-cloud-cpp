...
std::shared_ptr<CertificateManagerStub> CreateDefaultCertificateManagerStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CERTIFICATE_MANAGER_ENDPOINT"),
      std::move(endpoint_option), "certificatemanager.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::certificatemanager::v1::CertificateManager::NewStub(
            channel);
    stub = std::make_shared<DefaultCertificateManagerStub>(
