...
Like most Google Cloud Platform (GCP) services, Resource Settings API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,resourcesettings]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/resourcesettings/quickstart
...
[quickstart-link]: https://cloud.google.com/resource-manager/docs/resource-settings/manage-resource-settings
