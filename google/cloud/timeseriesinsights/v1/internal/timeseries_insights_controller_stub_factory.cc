...
CreateDefaultTimeseriesInsightsControllerStub(
  ...
  auto service_grpc_stub = google::cloud::timeseriesinsights::v1::TimeseriesInsightsController::NewStub(channel);
  std::shared_ptr<TimeseriesInsightsControllerStub> stub =
    std::make_shared<DefaultTimeseriesInsightsControllerStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<TimeseriesInsightsControllerAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeTimeseriesInsightsControllerTracingStub(std::move(stub));
  }
  return stub;
}
