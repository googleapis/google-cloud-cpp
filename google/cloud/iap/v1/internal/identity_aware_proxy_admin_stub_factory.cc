...
CreateDefaultIdentityAwareProxyAdminServiceStub(
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::iap::v1::IdentityAwareProxyAdminService::NewStub(
            channel);
    stub = std::make_shared<DefaultIdentityAwareProxyAdminServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeIdentityAwareProxyAdminServiceTracingStub(std::move(stub));
  }
  return stub;
}
