# HOWTO: using the Cloud Identity-Aware Proxy API C++ client in your project

This directory contains small examples showing how to use the Cloud Identity-Aware Proxy API C++
...
Like most Google Cloud Platform (GCP) services, Cloud Identity-Aware Proxy API requires that
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,iap]
   ```
...
1. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/iap/quickstart
...
[quickstart-link]: https://cloud.google.com/iap/docs/quickstart
