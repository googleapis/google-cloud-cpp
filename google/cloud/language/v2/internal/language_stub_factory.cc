...
std::shared_ptr<LanguageServiceStub>
CreateDefaultLanguageServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::language::v2::LanguageService::NewStub(channel);
  std::shared_ptr<LanguageServiceStub> stub =
    std::make_shared<DefaultLanguageServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<LanguageServiceAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeLanguageServiceTracingStub(std::move(stub));
  }
  return stub;
}
