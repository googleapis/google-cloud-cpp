# HOWTO: using the Cloud Monitoring API C++ client in your project

This directory contains small examples showing how to use the Cloud Monitoring API C++
...
Like most Google Cloud Platform (GCP) services, Cloud Monitoring API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,monitoring]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/monitoring/quickstart
...
[quickstart-link]: https://cloud.google.com/monitoring/docs/quickstart
