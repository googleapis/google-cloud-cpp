...
Options UptimeCheckServiceDefaultOptions(Options options) {
  ...
  if (!options.has<monitoring_v3::UptimeCheckServiceBackoffPolicyOption>()) {
    options.set<monitoring_v3::UptimeCheckServiceBackoffPolicyOption>(
        ExponentialBackoffPolicy(std::chrono::seconds(0), std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling, kBackoffScaling).clone());
  }
  if (!options.has<monitoring_v3::UptimeCheckServiceConnectionIdempotencyPolicyOption>()) {
    options.set<monitoring_v3::UptimeCheckServiceConnectionIdempotencyPolicyOption>(
