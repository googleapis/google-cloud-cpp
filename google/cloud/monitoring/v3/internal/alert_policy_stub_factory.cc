...
std::shared_ptr<AlertPolicyServiceStub>
CreateDefaultAlertPolicyServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::monitoring::v3::AlertPolicyService::NewStub(channel);
  std::shared_ptr<AlertPolicyServiceStub> stub =
    std::make_shared<DefaultAlertPolicyServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AlertPolicyServiceAuth>(
