...
std::shared_ptr<UptimeCheckServiceStub>
CreateDefaultUptimeCheckServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::monitoring::v3::UptimeCheckService::NewStub(channel);
  std::shared_ptr<UptimeCheckServiceStub> stub =
    std::make_shared<DefaultUptimeCheckServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<UptimeCheckServiceAuth>(
