...
Options SnoozeServiceDefaultOptions(Options options) {
  options = internal::PopulateCommonOptions(
      std::move(options), "GOOGLE_CLOUD_CPP_SNOOZE_SERVICE_ENDPOINT",
      "", "GOOGLE_CLOUD_CPP_SNOOZE_SERVICE_AUTHORITY",
  ...
  if (!options.has<monitoring_v3::SnoozeServiceBackoffPolicyOption>()) {
    options.set<monitoring_v3::SnoozeServiceBackoffPolicyOption>(
        ExponentialBackoffPolicy(std::chrono::seconds(0), std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling, kBackoffScaling).clone());
  }
  if (!options.has<monitoring_v3::SnoozeServiceConnectionIdempotencyPolicyOption>()) {
    options.set<monitoring_v3::SnoozeServiceConnectionIdempotencyPolicyOption>(
        monitoring_v3::MakeDefaultSnoozeServiceConnectionIdempotencyPolicy());
  }
