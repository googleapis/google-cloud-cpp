...
std::shared_ptr<QueryServiceStub>
CreateDefaultQueryServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::monitoring::v3::QueryService::NewStub(channel);
  std::shared_ptr<QueryServiceStub> stub =
    std::make_shared<DefaultQueryServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<QueryServiceAuth>(
