...
#ifndef GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_RETAIL_INTERNAL_USER_EVENT_CONNECTION_IMPL_H
#define GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_RETAIL_INTERNAL_USER_EVENT_CONNECTION_IMPL_H
 ...
 private:
  ...
  std::unique_ptr<BackoffPolicy> backoff_policy() {
    ...
    if (options.has<retail::UserEventServiceBackoffPolicyOption>()) {
      return options.get<retail::UserEventServiceBackoffPolicyOption>()->clone();
    }
    return backoff_policy_prototype_->clone();
  }

  std::unique_ptr<retail::UserEventServiceConnectionIdempotencyPolicy> idempotency_policy() {
    auto const& options = internal::CurrentOptions();
    if (options.has<retail::UserEventServiceConnectionIdempotencyPolicyOption>()) {
      return options.get<retail::UserEventServiceConnectionIdempotencyPolicyOption>()->clone();
    }
  ...
  std::unique_ptr<retail::UserEventServiceRetryPolicy const> retry_policy_prototype_;
  std::unique_ptr<BackoffPolicy const> backoff_policy_prototype_;
  std::unique_ptr<retail::UserEventServiceConnectionIdempotencyPolicy> idempotency_policy_;

  std::unique_ptr<PollingPolicy> polling_policy() {
    auto const& options = internal::CurrentOptions();
    if (options.has<retail::UserEventServicePollingPolicyOption>()) {
      return options.get<retail::UserEventServicePollingPolicyOption>()->clone();
    }
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_RETAIL_INTERNAL_USER_EVENT_CONNECTION_IMPL_H
