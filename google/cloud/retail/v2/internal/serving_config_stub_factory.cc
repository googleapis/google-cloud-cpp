...
std::shared_ptr<ServingConfigServiceStub> CreateDefaultServingConfigServiceStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SERVING_CONFIG_SERVICE_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::retail::v2::ServingConfigService::NewStub(channel);
    stub = std::make_shared<DefaultServingConfigServiceStub>(
        std::move(service_grpc_stub));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ServingConfigServiceAuth>(std::move(auth),
                                                      std::move(stub));
  }
