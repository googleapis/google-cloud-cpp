...
std::shared_ptr<CatalogServiceStub>
CreateDefaultCatalogServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::retail::v2::CatalogService::NewStub(channel);
  auto service_operations_stub = google::longrunning::Operations::NewStub(channel);
  std::shared_ptr<CatalogServiceStub> stub =
    std::make_shared<DefaultCatalogServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<CatalogServiceAuth>(
