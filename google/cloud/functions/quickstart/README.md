# HOWTO: using the Cloud Functions API C++ client in your project

This directory contains small examples showing how to use the Cloud Functions API C++
...
Like most Google Cloud Platform (GCP) services, Cloud Functions API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,functions]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/functions/quickstart
...
[quickstart-link]: https://cloud.google.com/functions/docs/quickstart
