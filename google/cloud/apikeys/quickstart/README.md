# HOWTO: using the API Keys API C++ client in your project

This directory contains small examples showing how to use the API Keys API C++
...
Like most Google Cloud Platform (GCP) services, API Keys API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,apikeys]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/apikeys/quickstart
...
[quickstart-link]: https://cloud.google.com/api-keys/docs/get-started-api-keys
