...
std::shared_ptr<ApiKeysStub> CreateDefaultApiKeysStub(
    google::cloud::CompletionQueue cq, Options& options) {
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_API_KEYS_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "apikeys.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::api::apikeys::v2::ApiKeys::NewStub(channel);
    stub = std::make_shared<DefaultApiKeysStub>(
