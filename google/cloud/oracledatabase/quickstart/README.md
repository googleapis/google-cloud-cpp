# HOWTO: using the Oracle Database@Google Cloud API C++ client in your project

This directory contains small examples showing how to use the Oracle
Database@Google Cloud API C++ client library in your own project. These
...
Like most Google Cloud Platform (GCP) services, Oracle Database@Google Cloud API
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/oracledatabase/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,oracledatabase]
   ```
