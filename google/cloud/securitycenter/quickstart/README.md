# HOWTO: using the Security Command Center API C++ client in your project

This directory contains small examples showing how to use the Security Command Center API C++
...
Like most Google Cloud Platform (GCP) services, Security Command Center API requires that
...
2. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,securitycenter]
   ```
...
3. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/securitycenter/quickstart
...
[quickstart-link]: https://cloud.google.com/security-command-center/docs/quickstart-security-command-center
