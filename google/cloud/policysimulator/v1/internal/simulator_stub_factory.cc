...
std::shared_ptr<SimulatorStub> CreateDefaultSimulatorStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SIMULATOR_ENDPOINT"),
      std::move(endpoint_option), "policysimulator.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultSimulatorStub>(
