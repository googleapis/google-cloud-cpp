...
std::shared_ptr<AccessApprovalStub>
CreateDefaultAccessApprovalStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::accessapproval::v1::AccessApproval::NewStub(channel);
  std::shared_ptr<AccessApprovalStub> stub =
    std::make_shared<DefaultAccessApprovalStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<AccessApprovalAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeAccessApprovalTracingStub(std::move(stub));
  }
  return stub;
}
