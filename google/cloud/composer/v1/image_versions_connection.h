...
class ImageVersionsRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<ImageVersionsRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `ImageVersionsConnection` that stops retrying after a
...
class ImageVersionsLimitedErrorCountRetryPolicy
 ...
 public:
  ImageVersionsLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<ImageVersionsRetryPolicy> clone() const override {
    return std::make_unique<ImageVersionsLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `ImageVersionsConnection` that stops retrying after
/// some wall clock time has elapsed.
class ImageVersionsLimitedTimeRetryPolicy : public ImageVersionsRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<ImageVersionsRetryPolicy> clone() const override {
    return std::make_unique<ImageVersionsLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 * - `google::cloud::composer_v1::ImageVersionsPolicyOptionList`
 *
