...
std::shared_ptr<ImageVersionsStub> CreateDefaultImageVersionsStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_IMAGE_VERSIONS_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "composer.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub = google::cloud::orchestration::airflow::service::
        v1::ImageVersions::NewStub(channel);
    stub = std::make_shared<DefaultImageVersionsStub>(
