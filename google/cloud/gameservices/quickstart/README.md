...
Like most Google Cloud Platform (GCP) services, Game Services API requires that
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,gameservices]
   ```
...
1. Configure CMake, if necessary, configure the directory where you installed
   ...
   ```bash
   cd $HOME/gooogle-cloud-cpp/google/cloud/gameservices/quickstart
