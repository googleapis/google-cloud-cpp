...
std::shared_ptr<ServiceUsageStub> CreateDefaultServiceUsageStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SERVICE_USAGE_ENDPOINT"),
      std::move(endpoint_option), "serviceusage.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultServiceUsageStub>(
