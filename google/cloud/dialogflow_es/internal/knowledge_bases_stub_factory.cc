...
#include "google/cloud/dialogflow_es/internal/knowledge_bases_auth_decorator.h"
...
std::shared_ptr<KnowledgeBasesStub>
CreateDefaultKnowledgeBasesStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dialogflow::v2::KnowledgeBases::NewStub(channel);
  ...
  std::shared_ptr<KnowledgeBasesStub> stub =
    std::make_shared<DefaultKnowledgeBasesStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<KnowledgeBasesAuth>(
