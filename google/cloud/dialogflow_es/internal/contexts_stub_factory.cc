...
std::shared_ptr<ContextsStub>
CreateDefaultContextsStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dialogflow::v2::Contexts::NewStub(channel);
  ...
  std::shared_ptr<ContextsStub> stub =
    std::make_shared<DefaultContextsStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ContextsAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeContextsTracingStub(std::move(stub));
  }
  return stub;
}
