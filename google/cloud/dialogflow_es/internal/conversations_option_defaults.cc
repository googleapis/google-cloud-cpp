...
Options ConversationsDefaultOptions(std::string const& location, Options options) {
  ...
  if (!options.has<dialogflow_es::ConversationsBackoffPolicyOption>()) {
    options.set<dialogflow_es::ConversationsBackoffPolicyOption>(
        ExponentialBackoffPolicy(std::chrono::seconds(0), std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling, kBackoffScaling).clone());
  }
  if (!options.has<dialogflow_es::ConversationsConnectionIdempotencyPolicyOption>()) {
    options.set<dialogflow_es::ConversationsConnectionIdempotencyPolicyOption>(
        dialogflow_es::MakeDefaultConversationsConnectionIdempotencyPolicy());
  }
