...
std::shared_ptr<ParticipantsStub>
CreateDefaultParticipantsStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::dialogflow::v2::Participants::NewStub(channel);
  ...
  std::shared_ptr<ParticipantsStub> stub =
    std::make_shared<DefaultParticipantsStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ParticipantsAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeParticipantsTracingStub(std::move(stub));
  }
  return stub;
}
