...
DefaultSessionEntityTypesStub::GetSessionEntityType(
  ...
  google::cloud::dialogflow::v2::GetSessionEntityTypeRequest const& request) {
    ...
    auto status =
        grpc_stub_->GetSessionEntityType(&context, request, &response);
    if (!status.ok()) {
      return google::cloud::MakeStatusFromRpcError(status);
    }
    return response;
...
Status
DefaultSessionEntityTypesStub::DeleteSessionEntityType(
  grpc::ClientContext& context, Options const&,
...
Status
DefaultSessionEntityTypesStub::CancelOperation(
  grpc::ClientContext& context, Options const&,
