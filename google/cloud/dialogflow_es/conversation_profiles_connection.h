...
class ConversationProfilesRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<ConversationProfilesRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `ConversationProfilesConnection` that stops retrying after
...
class ConversationProfilesLimitedErrorCountRetryPolicy
 ...
 public:
  ConversationProfilesLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<ConversationProfilesRetryPolicy> clone() const override {
    return std::make_unique<ConversationProfilesLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `ConversationProfilesConnection` that stops retrying
...
class ConversationProfilesLimitedTimeRetryPolicy
 ...
 public:
  ...
  std::unique_ptr<ConversationProfilesRetryPolicy> clone() const override {
    return std::make_unique<ConversationProfilesLimitedTimeRetryPolicy>(
        maximum_duration());
  }
