...
std::shared_ptr<CloudBuildStub> CreateDefaultCloudBuildStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CLOUD_BUILD_ENDPOINT"),
      std::move(endpoint_option), "cloudbuild.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::devtools::cloudbuild::v1::CloudBuild::NewStub(channel);
    stub = std::make_shared<DefaultCloudBuildStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeCloudBuildTracingStub(std::move(stub));
  }
  return stub;
}
