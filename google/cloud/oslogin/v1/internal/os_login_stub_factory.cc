...
std::shared_ptr<OsLoginServiceStub> CreateDefaultOsLoginServiceStub(
    google::cloud::CompletionQueue cq, Options& options) {
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_OS_LOGIN_SERVICE_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "oslogin.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::oslogin::v1::OsLoginService::NewStub(channel);
    stub = std::make_shared<DefaultOsLoginServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeOsLoginServiceTracingStub(std::move(stub));
  }
  return stub;
}
