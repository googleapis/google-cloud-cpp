...
std::shared_ptr<OsLoginServiceStub>
CreateDefaultOsLoginServiceStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::oslogin::v1::OsLoginService::NewStub(channel);
  std::shared_ptr<OsLoginServiceStub> stub =
    std::make_shared<DefaultOsLoginServiceStub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<OsLoginServiceAuth>(
