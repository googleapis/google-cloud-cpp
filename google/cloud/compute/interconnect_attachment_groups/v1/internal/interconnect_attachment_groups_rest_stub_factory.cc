...
CreateDefaultInterconnectAttachmentGroupsRestStub(Options const& options) {
  ...
  std::shared_ptr<InterconnectAttachmentGroupsRestStub> stub =
      std::make_shared<DefaultInterconnectAttachmentGroupsRestStub>(std::move(opts));
  stub = std::make_shared<InterconnectAttachmentGroupsRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<InterconnectAttachmentGroupsRestLogging>(
        std::move(stub),
