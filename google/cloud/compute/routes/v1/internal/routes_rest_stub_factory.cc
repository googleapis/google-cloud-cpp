...
#include "google/cloud/compute/routes/v1/internal/routes_rest_logging_decorator.h"
...
std::shared_ptr<RoutesRestStub>
CreateDefaultRoutesRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<RoutesRestStub> stub =
      std::make_shared<DefaultRoutesRestStub>(std::move(opts));
  stub = std::make_shared<RoutesRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RoutesRestLogging>(
        std::move(stub),
