...
CreateDefaultRegionTargetTcpProxiesRestStub(Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_TARGET_TCP_PROXIES_ENDPOINT"),
  ...
  std::shared_ptr<RegionTargetTcpProxiesRestStub> stub =
      std::make_shared<DefaultRegionTargetTcpProxiesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionTargetTcpProxiesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionTargetTcpProxiesRestLogging>(
