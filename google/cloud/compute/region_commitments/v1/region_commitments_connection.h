...
/// The retry policy for `RegionCommitmentsConnection`.
class RegionCommitmentsRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<RegionCommitmentsRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `RegionCommitmentsConnection` that stops retrying after a
...
class RegionCommitmentsLimitedErrorCountRetryPolicy
 ...
 public:
  RegionCommitmentsLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<RegionCommitmentsRetryPolicy> clone() const override {
    return std::make_unique<RegionCommitmentsLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `RegionCommitmentsConnection` that stops retrying after
...
class RegionCommitmentsLimitedTimeRetryPolicy
 ...
 public:
  ...
  std::unique_ptr<RegionCommitmentsRetryPolicy> clone() const override {
    return std::make_unique<RegionCommitmentsLimitedTimeRetryPolicy>(
        maximum_duration());
  }
