...
std::shared_ptr<BackendServicesRestStub> CreateDefaultBackendServicesRestStub(
    Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_BACKEND_SERVICES_ENDPOINT"),
  ...
  std::shared_ptr<BackendServicesRestStub> stub =
      std::make_shared<DefaultBackendServicesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<BackendServicesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<BackendServicesRestLogging>(
