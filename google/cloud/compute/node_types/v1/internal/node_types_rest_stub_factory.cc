...
#include "google/cloud/compute/node_types/v1/internal/node_types_rest_logging_decorator.h"
...
std::shared_ptr<NodeTypesRestStub> CreateDefaultNodeTypesRestStub(
    Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_NODE_TYPES_ENDPOINT"),
  ...
  std::shared_ptr<NodeTypesRestStub> stub =
      std::make_shared<DefaultNodeTypesRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<NodeTypesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<NodeTypesRestLogging>(
