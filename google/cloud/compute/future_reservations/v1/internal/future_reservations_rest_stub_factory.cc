...
#include "google/cloud/compute/future_reservations/v1/internal/future_reservations_rest_logging_decorator.h"
...
CreateDefaultFutureReservationsRestStub(Options const& options) {
  ...
  std::shared_ptr<FutureReservationsRestStub> stub =
      std::make_shared<DefaultFutureReservationsRestStub>(std::move(opts));
  stub = std::make_shared<FutureReservationsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<FutureReservationsRestLogging>(
