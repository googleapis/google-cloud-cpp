...
std::shared_ptr<RegionDiskTypesRestStub>
CreateDefaultRegionDiskTypesRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<RegionDiskTypesRestStub> stub =
      std::make_shared<DefaultRegionDiskTypesRestStub>(std::move(opts));
  stub = std::make_shared<RegionDiskTypesRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionDiskTypesRestLogging>(
        std::move(stub),
