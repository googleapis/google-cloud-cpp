...
CreateDefaultNetworkEdgeSecurityServicesRestStub(Options const& options) {
  ...
  std::shared_ptr<NetworkEdgeSecurityServicesRestStub> stub =
      std::make_shared<DefaultNetworkEdgeSecurityServicesRestStub>(std::move(opts));
  stub = std::make_shared<NetworkEdgeSecurityServicesRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<NetworkEdgeSecurityServicesRestLogging>(
        std::move(stub),
