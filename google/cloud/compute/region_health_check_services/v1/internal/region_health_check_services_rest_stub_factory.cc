...
CreateDefaultRegionHealthCheckServicesRestStub(Options& options) {
  ...
  std::shared_ptr<RegionHealthCheckServicesRestStub> stub =
      std::make_shared<DefaultRegionHealthCheckServicesRestStub>(
          std::move(stub_creation_opts));
  stub =
      std::make_shared<RegionHealthCheckServicesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionHealthCheckServicesRestLogging>(
