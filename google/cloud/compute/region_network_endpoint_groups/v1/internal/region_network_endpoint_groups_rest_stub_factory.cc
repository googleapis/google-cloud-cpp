...
CreateDefaultRegionNetworkEndpointGroupsRestStub(Options const& options) {
  ...
  std::shared_ptr<RegionNetworkEndpointGroupsRestStub> stub =
      std::make_shared<DefaultRegionNetworkEndpointGroupsRestStub>(std::move(opts));
  stub = std::make_shared<RegionNetworkEndpointGroupsRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionNetworkEndpointGroupsRestLogging>(
        std::move(stub),
