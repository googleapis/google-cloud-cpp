...
CreateDefaultRegionNetworkFirewallPoliciesRestStub(Options const& options) {
  ...
  std::shared_ptr<RegionNetworkFirewallPoliciesRestStub> stub =
      std::make_shared<DefaultRegionNetworkFirewallPoliciesRestStub>(std::move(opts));
  stub = std::make_shared<RegionNetworkFirewallPoliciesRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionNetworkFirewallPoliciesRestLogging>(
        std::move(stub),
