...
CreateDefaultExternalVpnGatewaysRestStub(Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_EXTERNAL_VPN_GATEWAYS_ENDPOINT"),
  ...
  std::shared_ptr<ExternalVpnGatewaysRestStub> stub =
      std::make_shared<DefaultExternalVpnGatewaysRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<ExternalVpnGatewaysRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<ExternalVpnGatewaysRestLogging>(
