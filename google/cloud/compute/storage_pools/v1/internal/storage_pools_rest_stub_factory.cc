...
#include "google/cloud/compute/storage_pools/v1/internal/storage_pools_rest_stub_factory.h"
...
std::shared_ptr<StoragePoolsRestStub>
CreateDefaultStoragePoolsRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<StoragePoolsRestStub> stub =
      std::make_shared<DefaultStoragePoolsRestStub>(std::move(opts));
  stub = std::make_shared<StoragePoolsRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<StoragePoolsRestLogging>(
        std::move(stub),
