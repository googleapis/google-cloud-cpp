...
CreateDefaultRegionInstantSnapshotsRestStub(Options const& options) {
  ...
  std::shared_ptr<RegionInstantSnapshotsRestStub> stub =
      std::make_shared<DefaultRegionInstantSnapshotsRestStub>(std::move(opts));
  stub = std::make_shared<RegionInstantSnapshotsRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionInstantSnapshotsRestLogging>(
        std::move(stub),
