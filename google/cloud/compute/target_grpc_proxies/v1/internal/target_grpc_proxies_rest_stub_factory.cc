...
CreateDefaultTargetGrpcProxiesRestStub(Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TARGET_GRPC_PROXIES_ENDPOINT"),
  ...
  std::shared_ptr<TargetGrpcProxiesRestStub> stub =
      std::make_shared<DefaultTargetGrpcProxiesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<TargetGrpcProxiesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<TargetGrpcProxiesRestLogging>(
