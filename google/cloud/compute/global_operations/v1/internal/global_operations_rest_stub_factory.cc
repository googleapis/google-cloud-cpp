...
#include "google/cloud/compute/global_operations/v1/internal/global_operations_rest_metadata_decorator.h"
...
std::shared_ptr<GlobalOperationsRestStub> CreateDefaultGlobalOperationsRestStub(
    Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_GLOBAL_OPERATIONS_ENDPOINT"),
  ...
  std::shared_ptr<GlobalOperationsRestStub> stub =
      std::make_shared<DefaultGlobalOperationsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<GlobalOperationsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<GlobalOperationsRestLogging>(
