...
std::shared_ptr<RegionsRestStub> CreateDefaultRegionsRestStub(
    Options& options) {
  ...
  auto service_endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGIONS_ENDPOINT"),
  ...
  std::shared_ptr<RegionsRestStub> stub =
      std::make_shared<DefaultRegionsRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<RegionsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionsRestLogging>(
