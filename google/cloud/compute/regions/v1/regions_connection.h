...
class RegionsRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<RegionsRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `RegionsConnection` that stops retrying after a
 ...
 */
class RegionsLimitedErrorCountRetryPolicy : public RegionsRetryPolicy {
 public:
  RegionsLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<RegionsRetryPolicy> clone() const override {
    return std::make_unique<RegionsLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `RegionsConnection` that stops retrying after
/// some wall clock time has elapsed.
class RegionsLimitedTimeRetryPolicy : public RegionsRetryPolicy {
 public:
  /**
 ...
 * sets in `RegionsClient`. This allows users to inject custom behavior
 * (e.g., with a Google Mock object) when writing tests that use objects of type
 * `RegionsClient`.
 *
 * To create a concrete instance, see `MakeRegionsConnection()`.
 *
 * For mocking, see `compute_regions_v1_mocks::MockRegionsConnection`.
 */
class RegionsConnection {
 public:
  virtual ~RegionsConnection() = 0;
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_COMPUTE_REGIONS_V1_REGIONS_CONNECTION_H
