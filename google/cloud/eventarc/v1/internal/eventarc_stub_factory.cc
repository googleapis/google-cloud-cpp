...
std::shared_ptr<EventarcStub> CreateDefaultEventarcStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_EVENTARC_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "eventarc.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultEventarcStub>(
