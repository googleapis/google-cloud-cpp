...
#include "google/cloud/beyondcorp/clientgateways/v1/internal/client_gateways_auth_decorator.h"
...
std::shared_ptr<ClientGatewaysServiceStub>
CreateDefaultClientGatewaysServiceStub(google::cloud::CompletionQueue cq,
                                       Options const& options) {
  auto auth = google::cloud::internal::CreateAuthenticationStrategy(
      std::move(cq), options);
  auto channel = auth->CreateChannel(options.get<EndpointOption>(),
                                     internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::beyondcorp::clientgateways::v1::
      ClientGatewaysService::NewStub(channel);
  std::shared_ptr<ClientGatewaysServiceStub> stub =
      std::make_shared<DefaultClientGatewaysServiceStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeClientGatewaysServiceTracingStub(std::move(stub));
  }
  return stub;
}
