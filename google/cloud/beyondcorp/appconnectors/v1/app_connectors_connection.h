...
class AppConnectorsServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<AppConnectorsServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `AppConnectorsServiceConnection` that stops retrying after
...
class AppConnectorsServiceLimitedErrorCountRetryPolicy
 ...
 public:
  AppConnectorsServiceLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<AppConnectorsServiceRetryPolicy> clone() const override {
    return std::make_unique<AppConnectorsServiceLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `AppConnectorsServiceConnection` that stops retrying
...
class AppConnectorsServiceLimitedTimeRetryPolicy
 ...
 public:
  ...
  std::unique_ptr<AppConnectorsServiceRetryPolicy> clone() const override {
    return std::make_unique<AppConnectorsServiceLimitedTimeRetryPolicy>(
        maximum_duration());
  }
