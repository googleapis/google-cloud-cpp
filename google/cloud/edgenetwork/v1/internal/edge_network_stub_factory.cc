...
std::shared_ptr<EdgeNetworkStub> CreateDefaultEdgeNetworkStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_EDGE_NETWORK_ENDPOINT"),
      std::move(endpoint_option), "edgenetwork.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultEdgeNetworkStub>(
        std::move(service_grpc_stub),
        google::longrunning::Operations::NewStub(channel));
  }
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<EdgeNetworkAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<EdgeNetworkMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<EdgeNetworkLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<TracingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeEdgeNetworkTracingStub(std::move(stub));
  }
  return stub;
}
