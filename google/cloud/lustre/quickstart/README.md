# HOWTO: using the Google Cloud Managed Lustre API C++ client in your project

This directory contains small examples showing how to use the Google Cloud
Managed Lustre API C++ client library in your own project. These instructions
...
Like most Google Cloud Platform (GCP) services, Google Cloud Managed Lustre API
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/lustre/quickstart
   bazel build ...
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,lustre]
   ```
