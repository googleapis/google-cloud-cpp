...
std::shared_ptr<SqlOperationsServiceConnection> MakeSqlOperationsServiceConnectionRest(
    Options options) {
  internal::CheckExpectedOptions<CommonOptionList, RestOptionList,
      UnifiedCredentialsOptionList, rest_internal::TargetApiVersionOption,
      SqlOperationsServicePolicyOptionList>(options, __func__);
  options = sql_v1_internal::SqlOperationsServiceDefaultOptions(
      std::move(options));
  auto background = std::make_unique<
      rest_internal::AutomaticallyCreatedRestBackgroundThreads>();
  auto stub = sql_v1_internal::CreateDefaultSqlOperationsServiceRestStub(
      options);
  return sql_v1_internal::MakeSqlOperationsServiceTracingConnection(
      std::make_shared<
          sql_v1_internal::SqlOperationsServiceRestConnectionImpl>(
