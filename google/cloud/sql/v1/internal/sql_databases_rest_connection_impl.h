...
class SqlDatabasesServiceRestConnectionImpl
 ...
 private:
  static std::unique_ptr<sql_v1::SqlDatabasesServiceRetryPolicy>
  retry_policy(Options const& options) {
    return options.get<sql_v1::SqlDatabasesServiceRetryPolicyOption>()->clone();
  }

  static std::unique_ptr<BackoffPolicy> backoff_policy(Options const& options) {
    return options.get<sql_v1::SqlDatabasesServiceBackoffPolicyOption>()->clone();
  }

  static std::unique_ptr<sql_v1::SqlDatabasesServiceConnectionIdempotencyPolicy>
  idempotency_policy(Options const& options) {
    return options.get<sql_v1::SqlDatabasesServiceConnectionIdempotencyPolicyOption>()->clone();
  }
