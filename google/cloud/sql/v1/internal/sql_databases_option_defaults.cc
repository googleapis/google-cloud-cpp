...
Options SqlDatabasesServiceDefaultOptions(Options options) {
  options = internal::PopulateCommonOptions(
      std::move(options), "GOOGLE_CLOUD_CPP_SQL_DATABASES_SERVICE_ENDPOINT",
      "", "GOOGLE_CLOUD_CPP_SQL_DATABASES_SERVICE_AUTHORITY",
  ...
  if (!options.has<sql_v1::SqlDatabasesServiceBackoffPolicyOption>()) {
    options.set<sql_v1::SqlDatabasesServiceBackoffPolicyOption>(
        ExponentialBackoffPolicy(std::chrono::seconds(0), std::chrono::seconds(1),
            std::chrono::minutes(5), kBackoffScaling, kBackoffScaling).clone());
  }
  if (!options.has<sql_v1::SqlDatabasesServiceConnectionIdempotencyPolicyOption>()) {
    options.set<sql_v1::SqlDatabasesServiceConnectionIdempotencyPolicyOption>(
        sql_v1::MakeDefaultSqlDatabasesServiceConnectionIdempotencyPolicy());
  }
