...
std::shared_ptr<FleetRoutingStub> CreateDefaultFleetRoutingStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_FLEET_ROUTING_ENDPOINT"),
      std::move(endpoint_option), "cloudoptimization.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultFleetRoutingStub>(
