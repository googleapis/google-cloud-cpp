...
std::shared_ptr<BatchControllerStub> CreateDefaultBatchControllerStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_BATCH_CONTROLLER_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    auto service_grpc_stub =
        google::cloud::dataproc::v1::BatchController::NewStub(channel);
    stub = std::make_shared<DefaultBatchControllerStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeBatchControllerTracingStub(std::move(stub));
  }
  return stub;
}
