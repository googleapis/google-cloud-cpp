# HOWTO: using the Cloud Commerce Consumer Procurement API C++ client in your project

This directory contains small examples showing how to use the Cloud Commerce
Consumer Procurement API C++ client library in your own project. These
...
Like most Google Cloud Platform (GCP) services, Cloud Commerce Consumer
...
1. Compile this example using Bazel:
   ...
   ```bash
   cd $HOME/google-cloud-cpp/google/cloud/commerce/quickstart
   bazel build ...
   ```
...
1. Run the example, changing the placeholder(s) to appropriate values:
   ...
   ```bash
   bazel run :quickstart -- [billing-account]
   ```
...
1. Install the dependencies with your favorite tools. As an example, if you use
   ...
   ```bash
   cd $HOME/vcpkg
   ./vcpkg install google-cloud-cpp[core,commerce]
   ```
...
1. Run the example, changing the placeholder(s) to appropriate values:
   ...
   ```bash
   .build/quickstart [billing-account]
   ```
...
[quickstart-link]: https://cloud.google.com/marketplace/docs/commitment-api-purchasing
