...
CreateDefaultConsumerProcurementServiceStub(
  ...
  auto service_grpc_stub = google::cloud::commerce::consumer::procurement::v1::ConsumerProcurementService::NewStub(channel);
  ...
  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ConsumerProcurementServiceAuth>(
        std::move(auth), std::move(stub));
  }
  stub = std::make_shared<ConsumerProcurementServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<ConsumerProcurementServiceLogging>(
        std::move(stub),
        options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeConsumerProcurementServiceTracingStub(std::move(stub));
  }
  return stub;
}
