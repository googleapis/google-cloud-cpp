...
class TenantServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<TenantServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `TenantServiceConnection` that stops retrying after a
...
class TenantServiceLimitedErrorCountRetryPolicy
 ...
 public:
  TenantServiceLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<TenantServiceRetryPolicy> clone() const override {
    return std::make_unique<TenantServiceLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `TenantServiceConnection` that stops retrying after
/// some wall clock time has elapsed.
class TenantServiceLimitedTimeRetryPolicy : public TenantServiceRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<TenantServiceRetryPolicy> clone() const override {
    return std::make_unique<TenantServiceLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 * - `google::cloud::talent_v4::TenantServicePolicyOptionList`
 *
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_TALENT_V4_TENANT_CONNECTION_H
