...
TEST(ObjectAccessControlTest, Parse) {
  ...
  auto actual = ObjectAccessControl::ParseFromJson(text);
...
TEST(ObjectAccessControlTest, IOStream) {
  ...
  auto meta = ObjectAccessControl::ParseFromJson(text);
...
/// @test Double check the well-known values defined in ObjectAccessControl.
TEST(ObjectAccessControlTest, WellKnownValues) {
  EXPECT_EQ("OWNER", ObjectAccessControl::ROLE_OWNER());
  EXPECT_EQ("READER", ObjectAccessControl::ROLE_READER());

  EXPECT_EQ("editors", ObjectAccessControl::TEAM_EDITORS());
  EXPECT_EQ("owners", ObjectAccessControl::TEAM_OWNERS());
  EXPECT_EQ("viewers", ObjectAccessControl::TEAM_VIEWERS());
}
}  // namespace
