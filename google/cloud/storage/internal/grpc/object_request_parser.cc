...
StatusOr<google::storage::v2::ComposeObjectRequest> ToProto(
  ...
  // Using `GetOption` with `value_or(false)` ensures that `delete_source_objects`
  // is only set to `true` if explicitly requested, preserving the default
  // behavior (no deletion) otherwise.
