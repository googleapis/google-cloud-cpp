...
TEST(ReadObjectRangeRequestTest, Simple) {
  ...
  request.set_modifier(storage::UserProject("my-project"));
  request.set_multiple_modifiers(storage::IfGenerationMatch(7),
...
TEST(ReadObjectRangeRequestTest, OStreamParameter) {
  ReadObjectRangeRequest request("my-bucket", "my-object");
  request.set_multiple_modifiers(Generation(3));
  std::ostringstream os;
  os << request;
