...
/**
 * A placeholder for backwards compatibility.
 *
 * This class only exists so the `storage::oauth2::*Credential<>` template
 * classes can continue to use the same default template parameters. This class
 * is not used beyond the template parameters.
 */
class CurlRequestBuilder {
 public:
  using RequestType = CurlRequest;
  CurlRequestBuilder() = default;
};
