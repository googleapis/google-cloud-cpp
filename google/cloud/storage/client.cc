...
Options DefaultOptions(Options opts) {
  ...
  auto metrics = GetEnv("GCP_STORAGE_CPP_ENABLE_OTEL_METRICS");
  if (!metrics || metrics->empty()) {
    metrics = GetEnv("GOOGLE_CLOUD_CPP_OPENTELEMETRY_METRICS");
  }
  if (metrics && !metrics->empty()) {
    o.set<OpenTelemetryMetricsOption>(true);
    o.set<storage_experimental::EnableOTelMetricsOption>(true);
  }

  if (o.has<storage_experimental::EnableOTelMetricsOption>()) {
    o.set<OpenTelemetryMetricsOption>(
        o.get<storage_experimental::EnableOTelMetricsOption>());
  }
