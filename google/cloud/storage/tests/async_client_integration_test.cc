...
TEST_F(AsyncClientIntegrationTest, StartAppendableUploadEmpty) {
  ...
  auto constexpr kBlockCount = 100000;
...
TEST_F(AsyncClientIntegrationTest, ComposeObjectSimple) {
  auto project_id = GetEnv("GOOGLE_CLOUD_PROJECT").value_or("");
  ASSERT_FALSE(project_id.empty());
  auto bucket_name = GetEnv("GOOGLE_CLOUD_CPP_STORAGE_TEST_BUCKET_NAME").value_or("");
  ASSERT_FALSE(bucket_name.empty());

  // Use the standard sync client to hit the real GCS endpoint for ComposeObject.
  auto client = MakeGrpcClient(project_id);
  auto source_name_1 = MakeRandomObjectName();
  auto source_name_2 = MakeRandomObjectName();
  auto composed_name = MakeRandomObjectName();

  auto const block1 = MakeRandomData(1024);
  auto const block2 = MakeRandomData(1024);

  auto insert1 = client.InsertObject(bucket_name, source_name_1, block1);
  ASSERT_STATUS_OK(insert1);
  ScheduleForDelete(*insert1);

  auto insert2 = client.InsertObject(bucket_name, source_name_2, block2);
  ASSERT_STATUS_OK(insert2);
  ScheduleForDelete(*insert2);

  std::vector<ComposeSourceObject> sources = {{source_name_1, {}, {}},
                                              {source_name_2, {}, {}}};

  auto composed = client.ComposeObject(bucket_name, sources, composed_name);
  ASSERT_STATUS_OK(composed);
  ScheduleForDelete(*composed);

  EXPECT_EQ(composed->size(), block1.size() + block2.size());
}
