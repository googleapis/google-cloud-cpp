...
struct MtlsObjectBasicCRUDIntegrationTest
 ...
 public:
  ...
  static Client MakeMtlsClient(std::string const& test_key_file_contents,
                               ...
                               std::vector<absl::string_view> const& ca_certs) {
    ...
    if (!ca_certs.empty()) {
      options.set<experimental::CAInMemoryOption>(ca_certs);
    }
...
TEST_F(MtlsObjectBasicCRUDIntegrationTest, CAInMemoryBasicCRUD) {
  if (!(GetEnv("GOOGLE_CLOUD_CPP_CLIENT_SSL_CERT_FILE").has_value() &&
        GetEnv("GOOGLE_CLOUD_CPP_CLIENT_SSL_KEY_FILE").has_value())) {
    GTEST_SKIP();
  }
  ...
  // Read in CA certs found on filesystem.
  auto ca_certs_text =
      ReadEnvVarFile("GOOGLE_CLOUD_CPP_STORAGE_TEST_ROOTS_PEM");
  ASSERT_STATUS_OK(ca_certs_text);

  // Create vector of CA certs in-memory.
  std::vector<absl::string_view> ca_certs;
  std::string begin_cert = "-----BEGIN CERTIFICATE-----";
  std::string end_cert = "-----END CERTIFICATE-----";
  size_t current = 0;
  size_t begin;
  while ((begin = ca_certs_text->find(begin_cert, current)) !=
         std::string::npos) {
    auto end = ca_certs_text->find(end_cert, begin);
    current = end + end_cert.length();
    ca_certs.emplace_back(&(*ca_certs_text)[begin], current - begin);
  }
  ASSERT_THAT(ca_certs, Not(IsEmpty()));

  auto client = MakeMtlsClient(*test_key_file_contents, *client_ssl_cert,
                               *client_ssl_key, ca_certs);
