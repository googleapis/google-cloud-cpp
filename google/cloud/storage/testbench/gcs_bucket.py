...
"""A class to simulate a GCS bucket in the client test bench."""
...
class GcsBucket(object):
    ...
    def __init__(self, gcs_url, name):
        ...
        self.insert_acl(
            gcs_service.canonical_entity_name('project-owners-123456789'),
            'OWNER')
        self.insert_acl(
            gcs_service.canonical_entity_name('project-editors-123456789'),
            'OWNER')
        self.insert_acl(
            gcs_service.canonical_entity_name('project-viewers-123456789'),
            'READER')
    ...
    def create_acl_entry(self, entity, role):
        ...
        :rtype: (str,dict)
        """
        entity = gcs_service.canonical_entity_name(entity)
        email = ''
    ...
    def insert_acl(self, entity, role):
        ...
        indexed = gcs_service.index_acl(self.metadata.get('acl', []))
    ...
    def get_acl(self, entity):
        ...
        for acl in self.metadata.get('acl', []):
            if acl.get('entity', '') == entity:
                return acl
        raise gcs_service.ErrorResponse('Entity %s not found in object %s' %
                                        (entity, self.name))
    ...
    def insert_default_object_acl(self, entity, role):
        ...
        # Replace or insert the entry.
        indexed = gcs_service.index_acl(
            self.metadata.get('defaultObjectAcl', []))
        indexed[entity] = {
            'bucket': self.name,
            'email': email,
            'entity': entity,
    ...
    def set_iam_policy(self, request):
        ...
        if policy.get('bindings') is None:
            raise gcs_service.ErrorResponse('Missing "bindings" field')
    ...
    @classmethod
    def insert_magic_bucket(cls, base_url):
        if gcs_service.bucket_count() == 0:
