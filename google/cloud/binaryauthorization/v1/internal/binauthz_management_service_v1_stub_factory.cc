...
CreateDefaultBinauthzManagementServiceV1Stub(
  ...
  auto service_grpc_stub = google::cloud::binaryauthorization::v1::BinauthzManagementServiceV1::NewStub(channel);
  std::shared_ptr<BinauthzManagementServiceV1Stub> stub =
    std::make_shared<DefaultBinauthzManagementServiceV1Stub>(std::move(service_grpc_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<BinauthzManagementServiceV1Auth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeBinauthzManagementServiceV1TracingStub(std::move(stub));
  }
  return stub;
}
