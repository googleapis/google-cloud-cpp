...
std::shared_ptr<IDSStub> CreateDefaultIDSStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(options.get<EndpointOption>(),
                                     internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::ids::v1::IDS::NewStub(channel);
  std::shared_ptr<IDSStub> stub = std::make_shared<DefaultIDSStub>(
  ...
  stub =
      std::make_shared<IDSMetadata>(std::move(stub), std::move(fixed_metadata));
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeIDSTracingStub(std::move(stub));
  }
  return stub;
}
