...
#include <vector>
   ...
   protected:
    static void SetUpTestSuite() {
      ...
      client_ = std::make_unique<Client>(MakeConnection(
          GetDatabase(), Options{}.set<GrpcCompressionAlgorithmOption>(
  ...
  TEST_F(ClientIntegrationTest, InsertAndCommit) {
    ...
    EXPECT_THAT(returned_rows, UnorderedElementsAre(
  ...
  TEST_F(ClientIntegrationTest, MultipleInserts) {
    ...
    auto& client = *client_;
    auto commit_result = client_->Commit(
        [&client](Transaction const& txn) -> StatusOr<Mutations> {
  ...
  TEST_F(ClientIntegrationTest, Commit) {
    ...
    // Delete SingerId 102.
    auto delete_result = client_->Commit(Mutations{
  ...
  TEST_F(ClientIntegrationTest, CommitAtLeastOnce) {
    ...
    auto delete_result = client_->CommitAtLeastOnce(
        ...
        Mutations{
            MakeDeleteMutation("Singers", KeySet().AddKey(MakeKey(202)))});
    if (delete_result) {
      EXPECT_LT(insert_result->commit_timestamp,
                delete_result->commit_timestamp);
    } else {
  ...
  TEST_F(ClientIntegrationTest, CommitAtLeastOnceBatched) {
    ...
    auto commit_results =
        client_->CommitAtLeastOnce(std::move(mutation_groups));
  ...
  TEST_F(ClientIntegrationTest, ExecuteQueryDml) {
    ...
    auto commit_result =
        client_->Commit([&client, &expected_rows](
  ...
  TEST_F(ClientIntegrationTest, QueryOptionsWork) {
    ...
    if (!UsingEmulator()) {
      EXPECT_TRUE(got_error)
          << "An invalid optimizer version should be an error";
  ...
  TEST_F(ClientIntegrationTest, DirectedReadWithinReadWriteTransaction) {
    ...
    EXPECT_THAT(commit,
                StatusIs(UsingEmulator() ? StatusCode::kFailedPrecondition
  ...
  TEST_F(ClientIntegrationTest, ExecuteBatchDmlMany) {
    ...
    auto commit_result = client_->Commit(
        [&client, &batch_result_left, &batch_result_right, &left,
  ...
  TEST_F(ClientIntegrationTest, ForeignKeyDeleteCascade) {
    ...
    auto insert_commit = client_->Commit(Mutations{
        MakeInsertMutation("Customers", {"CustomerId", "CustomerName"}, 1,
                           "FKCustomer"),
    ...
    auto delete_commit = client_->Commit(Mutations{
    ...
    // same mutation.
    auto conflict_commit = client_->Commit(Mutations{
        MakeInsertMutation("Customers", {"CustomerId", "CustomerName"}, 1,
                           "FKCustomer1"),
        MakeDeleteMutation("Customers", KeySet().AddKey(MakeKey(1)))});
    EXPECT_THAT(
    ...
    auto reinsert_commit = client_->Commit(Mutations{
        MakeInsertMutation("Customers", {"CustomerId", "CustomerName"}, 1,
                           "FKCustomer1"),
        MakeInsertMutation("Customers", {"CustomerId", "CustomerName"}, 2,
                           "FKCustomer2"),
        MakeInsertMutation("ShoppingCarts",
                           {"CartId", "CustomerId", "CustomerName"},  //
                           1, 1, "FKCustomer1")});
    ASSERT_STATUS_OK(reinsert_commit);
    auto reconflict_commit = client_->Commit(Mutations{
        MakeInsertMutation("ShoppingCarts",
                           {"CartId", "CustomerId", "CustomerName"},  //
                           1, 2, "FKCustomer2"),
        MakeDeleteMutation("Customers", KeySet().AddKey(MakeKey(1)))});
    ...
    // Query INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS and validate
    // DELETE_RULE.
  ...
  TEST_F(ClientIntegrationTest, BackwardsCompatibility) {
    auto connection =
        ::google::cloud::spanner::v1::MakeConnection(GetDatabase());
