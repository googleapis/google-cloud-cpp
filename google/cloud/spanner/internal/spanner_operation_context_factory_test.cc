...
TEST(SpannerOperationContextFactoryTest, HandlesNullProcessRandomId) {
  DefaultSpannerOperationContextFactory factory(100, nullptr);
  auto ctx = factory.ExecuteSql();
  grpc::ClientContext client_context;
  ctx->PreCall(client_context);
  EXPECT_THAT(ctx->RequestId().value_or(""),
              MatchesStdRegex(R"(^1\.[0-9a-f]{16}\.100\.0\.1\.1$)"));
}
