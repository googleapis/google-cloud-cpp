...
class SessionPool : public std::enable_shared_from_this<SessionPool> {
 public:
   ...
   * `session`.
   */
  StubAndChannel GetStub(Session const& session);
  StubAndChannel GetStub(Session const& session, TransactionContext& context);
 ...
 private:
  ...
  StubAndChannel GetStub(std::unique_lock<std::mutex>);
