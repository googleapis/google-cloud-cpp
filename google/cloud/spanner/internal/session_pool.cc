...
StubAndChannel SessionPool::GetStub(Session const& session) {
...
StubAndChannel SessionPool::GetStub(std::unique_lock<std::mutex>) {
  auto const& channel = *next_dissociated_stub_channel_;
  auto stub_and_channel = StubAndChannel{channel->stub, channel->channel_id};
