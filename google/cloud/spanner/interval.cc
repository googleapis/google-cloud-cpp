...
Timestamp Add(Timestamp const& ts, Interval const& intvl, absl::TimeZone tz) {
  auto ci = tz.At(*ts.get<absl::Time>());
  auto offset = intvl.offset_;  // split seconds and subseconds
...
Interval Diff(Timestamp const& ts1, Timestamp const& ts2, absl::TimeZone tz) {
  ...
  return Interval(0, 0, static_cast<std::int32_t>(days), offset);
}
