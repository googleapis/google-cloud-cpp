...
void CreateDatabaseWithEncryptionKey(
  ...
  auto db =
      client
          .CreateDatabase(database, std::move(extra_statements), encryption_key)
          .get();
  if (!db) throw std::runtime_error(db.status().message());
  std::cout << "Created database [" << database << "]\n";
...
void CreateBackup(google::cloud::spanner::DatabaseAdminClient client,
  ...
  auto now = absl::Now();
  auto expire_time =
      google::cloud::spanner::MakeTimestamp(now + absl::Hours(7)).value();
  auto version_time = google::cloud::spanner::MakeTimestamp(now).value();
  auto backup =
  ...
  std::cout << "Backup '" << backup->name() << "' of '" << database.FullName()
            ...
            << google::protobuf::util::TimeUtil::ToString(
                   backup->version_time())
            << " and of size " << backup->size_bytes() << " bytes"
            << " was created at "
