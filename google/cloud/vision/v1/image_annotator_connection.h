...
class ImageAnnotatorRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<ImageAnnotatorRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `ImageAnnotatorConnection` that stops retrying after a
...
class ImageAnnotatorLimitedErrorCountRetryPolicy
 ...
 public:
  ImageAnnotatorLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<ImageAnnotatorRetryPolicy> clone() const override {
    return std::make_unique<ImageAnnotatorLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `ImageAnnotatorConnection` that stops retrying after
/// some wall clock time has elapsed.
class ImageAnnotatorLimitedTimeRetryPolicy : public ImageAnnotatorRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<ImageAnnotatorRetryPolicy> clone() const override {
    return std::make_unique<ImageAnnotatorLimitedTimeRetryPolicy>(
        maximum_duration());
  }
 ...
 *
 * For mocking, see `vision_v1_mocks::MockImageAnnotatorConnection`.
 */
 ...
 * - `google::cloud::vision_v1::ImageAnnotatorPolicyOptionList`
 *
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_VISION_V1_IMAGE_ANNOTATOR_CONNECTION_H
