...
std::shared_ptr<ProductSearchStub>
CreateDefaultProductSearchStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(
    options.get<EndpointOption>(), internal::MakeChannelArguments(options));
  auto service_grpc_stub = google::cloud::vision::v1::ProductSearch::NewStub(channel);
  std::shared_ptr<ProductSearchStub> stub =
    std::make_shared<DefaultProductSearchStub>(
      std::move(service_grpc_stub),
      google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<ProductSearchAuth>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeProductSearchTracingStub(std::move(stub));
  }
  return stub;
}
