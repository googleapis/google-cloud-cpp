...
#include "google/cloud/vision/v1/internal/image_annotator_logging_decorator.h"
...
std::shared_ptr<ImageAnnotatorStub> CreateDefaultImageAnnotatorStub(
    google::cloud::CompletionQueue cq, Options& options) {
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_IMAGE_ANNOTATOR_ENDPOINT"),
      internal::FetchOption<EndpointOption>(options), "vision.googleapis.com",
      options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultImageAnnotatorStub>(
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeImageAnnotatorTracingStub(std::move(stub));
  }
  return stub;
}
