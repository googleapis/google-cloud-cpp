...
std::shared_ptr<CloudSchedulerStub> CreateDefaultCloudSchedulerStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_CLOUD_SCHEDULER_ENDPOINT"),
      std::move(endpoint_option), "cloudscheduler.googleapis.com", options);
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultCloudSchedulerStub>(
