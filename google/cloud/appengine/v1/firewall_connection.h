...
class FirewallRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance with the initial state, as-if no errors had been
  /// handled.
  virtual std::unique_ptr<FirewallRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `FirewallConnection` that stops retrying after a
 ...
 */
class FirewallLimitedErrorCountRetryPolicy : public FirewallRetryPolicy {
 public:
  FirewallLimitedErrorCountRetryPolicy(int maximum_failures)
  ...
  std::unique_ptr<FirewallRetryPolicy> clone() const override {
    return std::make_unique<FirewallLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/// A retry policy for `FirewallConnection` that stops retrying after
/// some wall clock time has elapsed.
class FirewallLimitedTimeRetryPolicy : public FirewallRetryPolicy {
 public:
  /**
