...
#include "google/cloud/appengine/v1/internal/firewall_stub_factory.h"
#include "google/cloud/appengine/v1/internal/firewall_auth_decorator.h"
#include "google/cloud/appengine/v1/internal/firewall_logging_decorator.h"
...
std::shared_ptr<FirewallStub> CreateDefaultFirewallStub(
  ...
  auto endpoint = internal::DetermineServiceEndpoint(
      internal::GetEnv("GOOGLE_CLOUD_CPP_FIREWALL_ENDPOINT"),
  ...
  if (!endpoint.ok()) {
    ...
    stub = std::make_shared<DefaultFirewallStub>(std::move(service_grpc_stub));
  }
  ...
  if (internal::TracingEnabled(options)) {
    stub = MakeFirewallTracingStub(std::move(stub));
  }
  return stub;
}
